...
class NpcCommand : CommandHandler {
    ...
    override suspend fun execute(session: PlayerSession, args: String, context: CommandContext) {
        ...
        if (npcManager == null) {
            session.send(ServerMessage.Notification(i18n.t(lang, "npc:server:system_unavailable")))
            return
        }
    ...
    private suspend fun handleList(
        ...
        for (instance in all) {
            ...
            session.send(
                ServerMessage.Notification("  ${s.id.take(8)} | ${s.name} | ${s.type} | $pos"))
        }
    }
    ...
    private suspend fun handleRemove(
        ...
        session.send(
            ServerMessage.Notification(i18n.t(lang, "npc:server:removed", instance.state.name)))
    }
