...
class EquipCommand : CommandHandler {
    ...
    override val command = "/equip"
    override val description = "Equip an armor piece."
    override val usage = "/equip <stuffName>"
    ...
    override suspend fun completeArg(
        ...
        if (argIndex == 0)
            context.stuffRegistry().keys.filter { it.startsWith(partial, ignoreCase = true) }
        else emptyList()
    ...
    override suspend fun execute(session: PlayerSession, args: String, context: CommandContext) {
        ...
        if (name.isBlank()) {
            val available = context.stuffRegistry().keys.sorted().joinToString(", ")
            session.send(Notification(i18n.t(lang, "equip:server:usage")))
            return
        }

        val slots = context.stuffRegistry()[name]
        if (slots == null) {
            val available = context.stuffRegistry().keys.sorted().joinToString(", ")
        ...
        val conflict =
            session.state.armors.firstOrNull { worn ->
                context.stuffRegistry()[worn]?.overlaps(slots) == true
            }
        if (conflict != null) {
...
class UnequipCommand : CommandHandler {
    ...
    override val command = "/unequip"
    override val description = "Remove an equipped armor piece."
    override val usage = "/unequip <stuffName>"
    override val autocompleteArgs = listOf(0)
