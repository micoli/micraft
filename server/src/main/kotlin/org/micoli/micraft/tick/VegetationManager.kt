...
import org.micoli.micraft.world.GrowthChain
...
import org.micoli.micraft.world.proceduralGenerator.vegetation.oakTreeBlocks
import org.micoli.micraft.world.proceduralGenerator.vegetation.pineTreeBlocks
import org.slf4j.LoggerFactory

private val log = LoggerFactory.getLogger(VegetationManager::class.java)
...
@Serializable private data class VegetationState(val blocks: List<GrowingBlock>)
...
class VegetationManager(
    ...
    private fun activate(pos: BlockPos, chain: GrowthChain) {
        if (chain.requiresVegetationHost) {
            ...
            if (belowY < 0 || !world.getBlockIfLoaded(pos.x, belowY, pos.z).isVegetationHost) {
                log.debug("VegetationManager: rejected {} — block below not a vegetation host", pos)
                return
            }
        }
    ...
    suspend fun tick(broadcast: suspend (ServerMessage) -> Unit) {
        ...
        for (pos in toAdvance) {
            ...
            if (nextStageIndex < chain.stages.size) {
                ...
                log.debug("VegetationManager: {} → {} (stage {})", pos, nextBlock, nextStageIndex)
            } else {
                activeBlocks.remove(pos)
    ...
    fun load() {
        ...
        runCatching {
            ...
            .onFailure { e ->
                log.warn("VegetationManager: failed to load {}: {}", savePath, e.message)
            }
    }
    ...
    fun save() {
        runCatching {
            ...
            .onFailure { e -> log.warn("VegetationManager: failed to save {}: {}", e.message) }
    }
