...
class I18nConfig(
                ...
                when (url.protocol) {
                    "file" ->
                        runCatching { dirs.add(Path.of(url.toURI())) }
                            .onFailure {
                                i18nLog.warn(
                                    "i18n: cannot resolve file URL {}: {}", url, it.message)
                            }
                    "jar" ->
                        runCatching {
                            ...
                            .onFailure {
                                i18nLog.warn("i18n: cannot read jar URL {}: {}", url, it.message)
                            }
                    else ->
                        i18nLog.warn("i18n: unsupported URL protocol {} for {}", url.protocol, url)
                }
            }
    ...
    fun reload() {
        ...
        for (dir in dirs) {
            if (!dir.exists()) {
                i18nLog.warn("i18n directory not found at {}, skipping", dir.toAbsolutePath())
                continue
            }
    ...
    private fun mergeYaml(
        ...
        runCatching {
                ...
                i18nLog.debug("i18n merged: {} from {} ({} keys)", locale, source, flat.size)
            }
            .onFailure { e ->
                i18nLog.warn("Failed to load i18n {}/{}: {}", source, locale, e.message)
            }
    }
