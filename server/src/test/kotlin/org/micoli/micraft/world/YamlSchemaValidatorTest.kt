...
class YamlSchemaValidatorTest {
    ...
    fun `valid YAML passes`() {
        val schema = tempSchema("""{"$${"$"}schema":"http://json-schema.org/draft-07/schema#","type":"object","required":["name"],"properties":{"name":{"type":"string"}}}""")
        val yaml = Files.createTempFile("micraft-yaml-test", ".yaml").also { it.toFile().writeText("name: hello") }
    ...
    fun `invalid YAML throws`() {
        val schema = tempSchema("""{"$${"$"}schema":"http://json-schema.org/draft-07/schema#","type":"object","required":["name"],"properties":{"name":{"type":"string"}}}""")
        val yaml = Files.createTempFile("micraft-yaml-test", ".yaml").also { it.toFile().writeText("age: 42") }
        assertFailsWith<IllegalStateException> { validateYaml(yaml, schema) }
    }

    @Test
    fun `missing YAML is silently skipped`() {
        val schema = tempSchema("""{"$${"$"}schema":"http://json-schema.org/draft-07/schema#","type":"object"}""")
