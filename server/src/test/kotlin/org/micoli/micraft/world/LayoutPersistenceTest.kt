...
class LayoutPersistenceTest {
    ...
    private fun minimalState() = PlayerState(
        id = "x",
        name = "Alice",
        pos = Vec3(0f, 0f, 0f),
    ...
    fun roundTrip_customLayouts_preservesData() {
        val custom = GameLayout(
            "compact",
            listOf(LayoutWidget(WidgetType.HUD, x = 5, y = 5, w = 10, h = 4))
        )
        val state = minimalState().copy(layouts = listOf(defaultLayout(), custom), activeLayout = "compact")
    ...
    fun deserialise_missingLayouts_usesDefault() {
        ...
        val json = """{"id":"x","name":"Alice","pos":{"x":0,"y":0,"z":0},"orientation":{"yaw":0,"pitch":0}}"""
