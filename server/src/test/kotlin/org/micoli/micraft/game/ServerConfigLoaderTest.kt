...
class ServerConfigLoaderTest {
    ...
    fun applyServerConfig_setsGameConstants() {
        ...
        applyServerConfig(config)
        assertEquals(-15f, GRAVITY)
        assertEquals(100L, TICK_MS)
        assertEquals(600, SAVE_INTERVAL_TICKS)
        assertEquals(16f, SPAWN_X)
        assertEquals(36_000L, TICKS_PER_DAY)
        assertEquals(10, TIME_BROADCAST_TICKS)
        assertEquals(5.0, MAX_INTERACTION_DISTANCE)
    }
}
