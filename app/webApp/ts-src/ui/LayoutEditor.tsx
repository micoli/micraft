...
const WIDGET_COLORS: Record<string, string> = {
  MINIMAP:      'rgba(60,120,200,0.75)',
  HUD:          'rgba(200,120,40,0.75)',
...
export function LayoutEditor({ open, layouts, activeLayout, onSave, onClose }: Props) {
  ...
  const updateWidgets = useCallback((updater: (ws: LayoutWidget[]) => LayoutWidget[]) => {
    setLocalLayouts(prev => prev.map(l =>
      l.name === localActive
        ? { ...l, widgets: updater(l.widgets) }
        : l
    ));
  }, [localActive]);
  ...
  const onMouseMove = (e: React.MouseEvent) => {
    ...
    updateWidgets(ws => ws.map(w => {
      if (w.type !== type) return w;
      if (mode === 'move') {
        return { ...w, x: Math.max(0, Math.min(48 - w.w, startX + dx)), y: Math.max(0, Math.min(48 - w.h, startY + dy)) };
      } else {
        return { ...w, w: Math.max(min.w, Math.min(48 - w.x, startW + dx)), h: Math.max(min.h, Math.min(48 - w.y, startH + dy)) };
      }
    }));
  };
  ...
  const handleReset = () => {
    setLocalLayouts(prev => prev.map(l =>
      l.name === localActive ? { ...l, widgets: [...DEFAULT_WIDGETS] } : l
    ));
  };
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, zIndex: 3000, background: 'rgba(0,0,0,0.85)', display: 'flex', flexDirection: 'column' }}
    ...
    >
      ...
      <div style={{
        ...
        background: 'rgba(0,0,0,0.7)', borderBottom: '1px solid rgba(255,255,255,0.15)',
        ...
        <select
          ...
          style={{ background: 'rgba(0,0,0,0.6)', color: '#fff', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 4, padding: '3px 6px', font: '12px monospace' }}
        >
          {localLayouts.map(l => <option key={l.name} value={l.name}>{l.name}</option>)}
        </select>
        ...
        <input
          ...
          style={{ background: 'rgba(0,0,0,0.6)', color: '#fff', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 4, padding: '3px 8px', font: '12px monospace', width: 120 }}
        />

        <button onClick={handleNewLayout} style={btnStyle}>+ New</button>
        <button onClick={handleReset} style={btnStyle}>Reset</button>

        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <button onClick={handleSave} style={{ ...btnStyle, background: 'rgba(60,140,60,0.8)', borderColor: 'rgba(100,200,100,0.5)' }}>Save</button>
          <button onClick={onClose} style={{ ...btnStyle, background: 'rgba(140,40,40,0.8)', borderColor: 'rgba(200,80,80,0.5)' }}>✕</button>
        </div>
      </div>
        ...
        {widgets.map(w => (
          <div
            ...
            style={{
              ...
              background: WIDGET_COLORS[w.type] ?? 'rgba(100,100,100,0.7)',
          ...
          >
            <span style={{ color: '#fff', font: 'bold 11px monospace', pointerEvents: 'none', textShadow: '1px 1px 2px rgba(0,0,0,0.8)' }}>
              {WIDGET_LABELS[w.type] ?? w.type}
              <br />
              <span style={{ font: '9px monospace', opacity: 0.7 }}>{w.x},{w.y} {w.w}×{w.h}</span>
            </span>
            {/* Resize handle */}
            <div
              style={{
...
const btnStyle: React.CSSProperties = {
  background: 'rgba(60,60,80,0.8)', color: '#fff',
  border: '1px solid rgba(255,255,255,0.25)', borderRadius: 4,
  padding: '4px 10px', font: '12px monospace', cursor: 'pointer',
};
