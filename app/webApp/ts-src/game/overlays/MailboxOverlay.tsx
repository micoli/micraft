...
interface MailboxOverlayProps {
  ...
  wallet?: number;
  onClose: () => void;
  /** How a mail action reaches the server — defaults to the wasm bridge's event queue (the game
   *  client). The `/hub` web companion has no wasm runtime to drain that queue, so it passes its
   *  own sender that encodes straight onto the `/hub` WebSocket instead — same UI, same actions,
   *  different transport. */
  onEvent?: (type: string, payload: string) => void;
  /** Radix modal behavior (focus trap + `aria-hidden` on everything outside the dialog) — on by
   *  default, matching the game where nothing behind the 3D scene should be reachable while this
   *  is open. The hub has no such background and its own persistent nav must stay reachable (and
   *  visible to accessibility-tree-based tooling), so it passes `false`. */
  modal?: boolean;
  /** "dialog" (default): the game's floating, closable window. "page": no Dialog/Portal at all —
   *  fills whatever block-level container it's rendered in, for the hub where each screen owns
   *  the whole main content area rather than floating over one. */
  chrome?: "dialog" | "page";
}
...
export function MailboxOverlay({
  ...
  const title = (
    <>
      ✉ Mailbox
      {unread > 0 && <span className="ml-2 text-xs bg-red-600 text-white rounded-full px-2 py-0.5">{unread}</span>}
    </>
  );
  ...
  const body = (
    <>
      ...
      <div className="flex items-center gap-3 px-5 py-3 border-b border-white/10">
        {chrome === "page" ? (
          <div className="font-mono text-lg font-semibold text-white/90 flex-1">{title}</div>
        ) : (
          <DialogTitle className="font-mono text-lg flex-1">{title}</DialogTitle>
        )}
        {view !== "inbox" && (
              ...
              <composeForm.Field name="to">
                {(field) => (
                  <Input
                    ...
                    onFocus={() => field.state.value.length >= 1 && suggestions.length > 0 && setShowSuggestions(true)}
                ...
                .map(([type, qty]) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="flex-1 text-[11px] text-white/80 truncate font-mono">{meta?.label ?? type}</span>
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()} modal={modal}>
      <DialogContent
