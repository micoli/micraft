...
export function registerCamera(): void {
  window.mcGetCameraPositionX = (camera: Camera): number => camera.position.x;
  window.mcGetCameraPositionY = (camera: Camera): number => camera.position.y;
  window.mcGetCameraPositionZ = (camera: Camera): number => camera.position.z;

  window.mcGetCameraDir3DX = (camera: Camera): number => camera.getForwardRay(1).direction.x;
  window.mcGetCameraDir3DY = (camera: Camera): number => camera.getForwardRay(1).direction.y;
  window.mcGetCameraDir3DZ = (camera: Camera): number => camera.getForwardRay(1).direction.z;

  window.mcGetCameraForwardX = (camera: Camera): number => {
    const d = camera.getForwardRay(1).direction;
    const l = Math.sqrt(d.x * d.x + d.z * d.z) || 1;
    return d.x / l;
  };

  window.mcGetCameraForwardZ = (camera: Camera): number => {
    const d = camera.getForwardRay(1).direction;
    const l = Math.sqrt(d.x * d.x + d.z * d.z) || 1;
    return d.z / l;
  };
  ...
  window.mcSetupDebugCameraKeys = (camera: Camera, scene: Scene, bx: number, by: number, bz: number): void => {
    ...
    const lock = (px: number, py: number, pz: number): void => {
      if (window.__debugCamObserver) (scene as any).onBeforeRenderObservable.remove(window.__debugCamObserver);
      window.__debugCamObserver = (scene as any).onBeforeRenderObservable.add(() => {
