...
import { getApiFurnitures } from "../../../generated/api/requests";
import type { OrgMicoliMicraftPlaceableFurnitureFurnitureDefinition } from "../../../generated/api/requests";
...
type FurnituresTabProps = {
  selectedKey: string | null;
  onSelectKey: (key: string | null, options?: { replace?: boolean }) => void;
};

type FurnitureEntry = { name: string; def: OrgMicoliMicraftPlaceableFurnitureFurnitureDefinition };

function bbmodelUrl(name: string) {
  return `/api/models/furnitures/${name}/${name}.bbmodel`;
}
...
function parseSelectedKey(key: string | null): {
  ...
  if (!key) return { furniture: null, animFullName: null, paused: false };
  const [furniture, animFullName, paused] = key.split(":");
  return { furniture, animFullName: animFullName || null, paused: paused === "1" };
}

function buildKey(furniture: string, animFullName: string, paused: boolean) {
  return `${furniture}:${animFullName}:${paused ? "1" : "0"}`;
}

export function FurnituresTab({ selectedKey, onSelectKey }: FurnituresTabProps) {
  const t = useT();
  const {
  ...
  useEffect(() => {
    getApiFurnitures({ throwOnError: true })
      .then((res) => {
  ...
  const selected = initialFurniture ? (entries.find((e) => e.name === initialFurniture) ?? null) : null;

  const hasAutoSelected = useRef(false);
  useEffect(() => {
    if (hasAutoSelected.current || initialFurniture || filtered.length === 0) return;
    hasAutoSelected.current = true;
    onSelectKey(buildKey(filtered[0].name, "still", false));
  }, [filtered, initialFurniture, onSelectKey]);
  ...
  const selectFurniture = (name: string) => onSelectKey(buildKey(name, "still", false));
    ...
    <div className="flex h-full overflow-hidden">
      <aside className="w-56 shrink-0 flex flex-col border-r border-[#2E3A4E] overflow-hidden">
        ...
        <SidebarList
          ...
          onSelect={(e) => selectFurniture(e.name)}
        />
      </aside>
      {selected ? (
          ...
          <div className="flex gap-6">
            <div className="flex flex-col items-center gap-2">
              <BbmodelAnimationViewer
                bbmodel={bbmodel}
                animFullName={selectedAnim?.fullName ?? "still"}
                paused={paused}
                standaloneItem
                width={280}
                height={280}
            ...
            <div className="max-w-sm flex-1">
              ...
              <PropRow label={t("administration.furnitureBbmodelFile")} value={selected.def.bbmodelFile} />
              <PropRow label={t("administration.furnitureWidth")} value={selected.def.width} />
              <PropRow label={t("administration.furnitureHeight")} value={selected.def.height} />
              <PropRow label={t("administration.furnitureRotatable")} value={selected.def.rotatable ? "✓" : "✗"} />
              {selectedAnim && (
