...
const FACE_SHADES = [0.95, 0.95, 0.85, 0.85, 1.00, 0.80];

interface FaceGroup {
  p: number[]; n: number[]; u: number[]; c: number[]; i: number[]; v: number;
}
interface ChunkBuf {
  key: string;
...
function emitCrossSprite(wx: number, wy: number, wz: number, mk: string, grp: Record<string, FaceGroup>, uv: number[], ao: number): void {
...
export function registerChunks(): void {
  ...
  window.mcChunkEnd = (scene: Scene, materials: Record<string, Material>): void => {
    ...
    for (const mk of Object.keys(buf.groups)) {
      ...
      vd.positions = g.p; vd.normals = g.n; vd.uvs = g.u; vd.colors = g.c; vd.indices = g.i;
