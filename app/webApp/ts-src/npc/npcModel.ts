...
export function registerNpcModel(): void {
  window.mcInitNpcModels = (npcTypesJson: string): void => {
    ...
    try { typeToFile = JSON.parse(npcTypesJson); } catch { return; }

    const entries = Object.entries(typeToFile);
    if (entries.length === 0) { window.__mc.npcModelsReady = true; return; }
  ...
  window.mcIsNpcModelsReady = (): boolean =>
    !!(window.__mc && window.__mc.npcModelsReady);
  ...
  window.mcCreateNpcModel = (scene: Scene, npcType: string): McPlayerModel | null => {
    ...
    if (!bbmodel) { console.warn('[MiCraft] NPC bbmodel not found for type:', npcType); return null; }
    return window.mcCreatePlayerModelFromBbmodel(bbmodel, scene);
  };

  window.mcSetNpcTransform = (
    model: McPlayerModel, x: number, y: number, z: number, yaw: number, isWalking: boolean,
  ): void => {
