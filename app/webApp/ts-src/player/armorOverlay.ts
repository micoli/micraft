...
export function registerArmorOverlay(): void {
  window.mcInitArmorModel = (name: string): void => {
    window.__mc = window.__mc || ({} as any);
    window.__mc.armorBbmodels = window.__mc.armorBbmodels || {};
    if (window.__mc.armorBbmodels[name]) return;
    fetch(`/api/models/stuff/${name}/${name}.bbmodel`)
      .then((r) => r.json())
      .then((data: BbModel) => {
        window.__mc.armorBbmodels[name] = data;
  ...
  window.mcIsArmorModelReady = (name: string): boolean => !!window.__mc?.armorBbmodels?.[name];

  window.mcAttachArmor = (model: McPlayerModel, armorName: string, scene: Scene): void => {
    if (model.equippedArmors[armorName]) return; // already attached

    const bbmodel = window.__mc?.armorBbmodels?.[armorName];
    if (!bbmodel) return;
    ...
    function walkOutliner(nodes: BbModel["outliner"], pivotKey: string | null | undefined, groupHidden: boolean): void {
      if (!nodes) return;
    ...
    for (const el of bbmodel.elements) {
      ...
      const mesh = BABYLON.MeshBuilder.CreateBox(
        `armor_${armorName}_${el.name}`,
        {
          ...
          faceUV: window.__mcSkinFaceUV(el.faces, W, H) as any,
        },
        scene,
      );
      mesh.material = mat;
  ...
  window.mcDetachArmor = (model: McPlayerModel, armorName: string): void => {
  ...
  window.mcDetachAllArmors = (model: McPlayerModel): void => {
