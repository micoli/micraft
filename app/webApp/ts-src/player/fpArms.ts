...
export function registerFPArms(): void {
  window.mcCreateFPArms = (scene: Scene, camera: Camera): McFPArms | null => {
    ...
    const W = bbmodel.resolution.width, H = bbmodel.resolution.height;
    const SCALE = 1 / 16;

    const groupMap: Record<string, BbModelGroup> = {};
    bbmodel.groups.forEach(g => { groupMap[g.name] = g; });
    ...
    for (const name of ['rightArm', 'leftArm'] as const) {
      ...
      if (!el) { console.warn('[MiCraft] FP arms: missing element', name); continue; }
      ...
      const mesh = BABYLON.MeshBuilder.CreateBox(`${name}_fp`, {
  ...
  window.mcUpdateFPArms = (fpArms: McFPArms, isWalking: boolean): void => {
    if (!fpArms) return;
    const DEG = Math.PI / 180;
    for (const p of fpArms.pivots) {
      if (!isWalking) { p.node.rotation.x = 0; continue; }
  ...
  window.mcSetFPArmsVisible = (fpArms: McFPArms, visible: boolean): void => {
    if (!fpArms) return;
    fpArms.meshes.forEach(m => { m.isVisible = visible; });
  };

  window.mcDisposeFPArms = (fpArms: McFPArms): void => {
    if (!fpArms) return;
  ...
  window.mcDebugFPArms = (x?: number, y?: number, z?: number): void => {
    const fa = window.__mcCurrentFPArms;
