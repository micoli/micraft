...
- `npm audit` on `app/webApp/ts-src` is **clean**; the `overrides` block in
  `package.json` forces patched `uuid` / `js-yaml`.
- **`kotlin-js-store/yarn.lock`** (webpack toolchain, regenerated by the Kotlin
  Gradle Plugin) is kept clean via `resolution(...)` entries in the root
  `build.gradle.kts`. Refresh after any Kotlin bump:
  `./gradlew kotlinUpgradeYarnLock` then re-run OSV-Scanner; drop a `resolution`
  once the KGP-pinned version already includes the fix.
- OSV-Scanner runs on both lockfiles in CI and is blocking.
