// Builds eel.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "eel.bbmodel");

const SEED = 20260911 + 16;
...
// ---- Colors -----------------------------------------------------------------
const SKIN_DARK = [46, 62, 54];
const SKIN_BELLY = [150, 168, 150];
const EYE_DARK = [12, 10, 8];

// ---- Geometry: corps long segmenté, tête plate, pas de pattes -------------------
const elements = [
  { name: "seg0", from: [-1, 7, -8], to: [1, 9, -5], bone: "body" },
  { name: "seg1", from: [-1, 7, -5], to: [1, 9, -1], bone: "body" },
  { name: "seg2", from: [-1, 7, -1], to: [1, 9, 4], bone: "body" },
  { name: "seg3", from: [-1, 7, 4], to: [1, 9, 8], bone: "body" },
  { name: "tailTip", from: [0, 7, 8], to: [1, 8, 10], bone: "body" },
];

const bones = { body: { origin: [0, 8, 0], rotation: [0, 0, 0] } };
...
function paintSmooth(rect, belly) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++)
    for (let x = x1; x < x2; x++) setPx(x, y, jitter(belly ? SKIN_BELLY : SKIN_DARK, 5));
}

for (const el of builtElements) {
  for (const face of faceOrder) paintSmooth(el.faces[face].uv, face === "down");
}

const headEl = builtElements.find((e) => e.name === "seg0");
{
  const [x1, y1, x2, y2] = headEl.faces.north.uv;
  const w = x2 - x1;
  setPx(x1 + Math.max(0, Math.round(w * 0.25)), y1, EYE_DARK);
  setPx(x1 + Math.max(0, Math.round(w * 0.75)), y1, EYE_DARK);
}
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = SKIN_DARK[0];
    canvas[i + 1] = SKIN_DARK[1];
    canvas[i + 2] = SKIN_DARK[2];
    canvas[i + 3] = 255;
  }
}
