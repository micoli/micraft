// Builds hyena.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "hyena.bbmodel");

const SEED = 20260911 + 27;
...
// ---- Colors -----------------------------------------------------------------
const BASE = [168, 142, 102];
const SPOT = [108, 84, 56];
const CREST = [78, 62, 44];
const EYE_AMBER = [190, 150, 50];

// ---- Geometry: silhouette penchée vers l'avant, épaules hautes, arrière bas -----
const elements = [
  { name: "frontBody", from: [-2, 7, -3], to: [2, 11, 4], bone: "body" },
  { name: "rearBody", from: [-2, 5, -6], to: [2, 9, -3], bone: "body" },
  { name: "head", from: [-3, 8, -9], to: [3, 12, -6], bone: "head" },
  { name: "crest", from: [-1, 11, -2], to: [1, 12, 2], bone: "body" },
  { name: "tail", from: [-1, 6, 4], to: [1, 8, 7], bone: "tail" },
  { name: "frontLegL", from: [-2, 0, -3], to: [-1, 7, -2], bone: "frontLegL" },
  { name: "frontLegR", from: [1, 0, -3], to: [2, 7, -2], bone: "frontLegR" },
  { name: "backLegL", from: [-2, 0, 3], to: [-1, 5, 4], bone: "backLegL" },
  { name: "backLegR", from: [1, 0, 3], to: [2, 5, 4], bone: "backLegR" },
];

const bones = {
  body: { origin: [0, 9, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 10, -6], rotation: [0, 0, 0] },
  tail: { origin: [0, 7, 4], rotation: [-10, 0, 0] },
  frontLegL: { origin: [-1.5, 7, -2.5], rotation: [0, 0, 0] },
  frontLegR: { origin: [1.5, 7, -2.5], rotation: [0, 0, 0] },
  backLegL: { origin: [-1.5, 5, 3.5], rotation: [0, 0, 0] },
  backLegR: { origin: [1.5, 5, 3.5], rotation: [0, 0, 0] },
};
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "crest") paintFlat(rect, CREST, 5);
    else paintSpotted(rect);
  }
}

for (const el of builtElements) {
  if (el.name === "crest") continue;
