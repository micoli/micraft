// Builds jackal.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "jackal.bbmodel");

const SEED = 20260911 + 4;
...
// ---- Colors -----------------------------------------------------------------
const SAND = [196, 168, 120];
const SAND_DARK = [164, 138, 96];
const MUZZLE_DARK = [110, 90, 64];
const EYE_AMBER = [190, 150, 40];

// ---- Geometry: fin, oreilles larges, museau fin, queue basse touffue -------------
const elements = [
  { name: "body", from: [-2, 6, -5], to: [2, 10, 4], bone: "body" },
  { name: "head", from: [-2, 7, -8], to: [2, 10, -5], bone: "head" },
  { name: "snout", from: [-1, 7, -9], to: [1, 8, -8], bone: "head" },
  { name: "earL", from: [-2, 10, -7], to: [-1, 12, -6], bone: "head" },
  { name: "earR", from: [1, 10, -7], to: [2, 12, -6], bone: "head" },
  { name: "tail", from: [-1, 7, 4], to: [1, 9, 8], bone: "tail" },
...
const bones = {
  ...
  tail: { origin: [0, 8, 4], rotation: [-10, 0, 0] },
...
function paintFur(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = SAND;
      const mott = valueNoise(x, y, 3);
      if (mott > 0.62) base = SAND_DARK;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "snout" || el.name.includes("Leg")) paintFlat(rect, MUZZLE_DARK, 6);
    else paintFur(rect);
  }
}
