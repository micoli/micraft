// Builds owl.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "owl.bbmodel");

const SEED = 20260911 + 2;
...
// ---- Colors -----------------------------------------------------------------
const BROWN = [122, 92, 60];
const BROWN_DARK = [90, 66, 42];
const CREAM = [214, 196, 158];
const EYE_YELLOW = [230, 200, 40];
const BEAK = [46, 36, 24];
const LEG_COLOR = [178, 148, 92];

// ---- Geometry: trapu, tête intégrée sans cou, ailes repliées le long du corps ---
const elements = [
  { name: "body", from: [-3, 6, -4], to: [3, 14, 4], bone: "body" },
  { name: "head", from: [-3, 14, -4], to: [3, 20, 3], bone: "head" },
  { name: "beak", from: [-1, 15, -5], to: [1, 17, -4], bone: "head" },
  { name: "wingL", from: [-4, 7, -3], to: [-3, 13, 3], bone: "wingL" },
  { name: "wingR", from: [3, 7, -3], to: [4, 13, 3], bone: "wingR" },
  { name: "legL", from: [-2, 0, -1], to: [-1, 6, 1], bone: "legL" },
  { name: "legR", from: [1, 0, -1], to: [2, 6, 1], bone: "legR" },
];

const bones = {
  body: { origin: [0, 6, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 14, -1], rotation: [0, 0, 0] },
  wingL: { origin: [-3, 13, 0], rotation: [0, 0, 0] },
  wingR: { origin: [3, 13, 0], rotation: [0, 0, 0] },
  legL: { origin: [-1.5, 6, 0], rotation: [0, 0, 0] },
  legR: { origin: [1.5, 6, 0], rotation: [0, 0, 0] },
};
...
function paintMottled(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = BROWN;
      const mott = valueNoise(x, y, 2.5);
      if (mott > 0.55) base = BROWN_DARK;
      setPx(x, y, jitter(base, 10));
    }
  }
}
function paintCream(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = CREAM;
      const mott = valueNoise(x, y, 2.5);
      if (mott > 0.7) base = base.map((c, i) => Math.round(c * 0.9 + BROWN[i] * 0.1));
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "beak") paintFlat(rect, BEAK, 4);
    else if (el.name === "legL" || el.name === "legR") paintFlat(rect, LEG_COLOR, 6);
    else if (el.name === "head" && face === "north") paintCream(rect);
    else paintMottled(rect);
  }
}

// cheap AO
for (const el of builtElements) {
  if (el.name === "beak") continue;
...
// Eyes on head's north (face disk), painted last
const headEl = builtElements.find((e) => e.name === "head");
{
  const [x1, y1, x2, y2] = headEl.faces.north.uv;
  const w = x2 - x1, h = y2 - y1;
  const eyeW = Math.max(1, Math.round(w * 0.26));
  const eyeH = Math.max(1, Math.round(h * 0.3));
  const ey = y1 + Math.round(h * 0.3);
  const exL = x1 + Math.round(w * 0.08);
  const exR = x2 - Math.round(w * 0.08) - eyeW;
  for (let y = ey; y < ey + eyeH; y++) {
    for (let x = exL; x < exL + eyeW; x++) setPx(x, y, EYE_YELLOW);
    for (let x = exR; x < exR + eyeW; x++) setPx(x, y, EYE_YELLOW);
  }
}
