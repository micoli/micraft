// Builds golem.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "golem.bbmodel");

const SEED = 20260911 + 22;
...
// ---- Colors -----------------------------------------------------------------
const STONE = [110, 108, 104];
const STONE_DARK = [80, 78, 74];
const CRACK = [224, 120, 40];
const EYE_GLOW = [230, 140, 50];

// ---- Geometry: trapu et cubique, proportions cubiques marquées ------------------
const elements = [
...
function paintRock(rect, isTorso) {
  ...
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      ...
      if (isTorso) {
        const crack = valueNoise(x + 50, y + 50, 4);
        if (crack > 0.82) px = CRACK;
      }
      setPx(x, y, px);
    }
  }
