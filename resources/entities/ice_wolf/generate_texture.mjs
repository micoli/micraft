// Builds ice_wolf.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "ice_wolf.bbmodel");

const SEED = 20260911 + 13;
...
// ---- Colors -----------------------------------------------------------------
const ICE_WHITE = [225, 232, 238];
const ICE_BLUE = [172, 194, 214];
const CRYSTAL = [190, 224, 236];
const EYE_ICE = [130, 210, 232];

// ---- Geometry: gabarit loup, fourrure givrée, cristaux dorsaux ------------------
const elements = [
  { name: "body", from: [-3, 7, -7], to: [3, 13, 6], bone: "body" },
  { name: "head", from: [-3, 9, -11], to: [3, 13, -7], bone: "head" },
  { name: "snout", from: [-1, 9, -13], to: [1, 11, -11], bone: "head" },
  { name: "earL", from: [-2, 13, -10], to: [-1, 15, -9], bone: "head" },
  { name: "earR", from: [1, 13, -10], to: [2, 15, -9], bone: "head" },
  { name: "crystal1", from: [-1, 13, -2], to: [1, 15, 0], bone: "body" },
  { name: "crystal2", from: [-1, 13, 1], to: [1, 15, 3], bone: "body" },
  { name: "tail", from: [-1, 8, 6], to: [1, 10, 11], bone: "tail" },
  { name: "frontLegL", from: [-2, 0, -5], to: [0, 7, -3], bone: "frontLegL" },
  { name: "frontLegR", from: [0, 0, -5], to: [2, 7, -3], bone: "frontLegR" },
  { name: "backLegL", from: [-2, 0, 3], to: [0, 7, 5], bone: "backLegL" },
  { name: "backLegR", from: [0, 0, 3], to: [2, 7, 5], bone: "backLegR" },
];

const bones = {
  body: { origin: [0, 13, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 13, -7], rotation: [0, 0, 0] },
  tail: { origin: [0, 10, 6], rotation: [-15, 0, 0] },
  frontLegL: { origin: [-1, 7, -4], rotation: [0, 0, 0] },
  frontLegR: { origin: [1, 7, -4], rotation: [0, 0, 0] },
  backLegL: { origin: [-1, 7, 4], rotation: [0, 0, 0] },
  backLegR: { origin: [1, 7, 4], rotation: [0, 0, 0] },
};
...
function paintFrostFur(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = ICE_WHITE;
      const mott = valueNoise(x, y, 2.5);
      if (mott > 0.6) base = ICE_BLUE;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name.startsWith("crystal")) paintFlat(rect, CRYSTAL, 4);
    else paintFrostFur(rect);
  }
}
...
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  const eyeH = Math.max(1, Math.round(h * 0.2));
  const ey = y1 + Math.round(h * 0.3);
  const exL = x1 + Math.round(w * 0.1);
  const exR = x2 - Math.round(w * 0.1) - eyeW;
  for (let y = ey; y < ey + eyeH; y++) {
    for (let x = exL; x < exL + eyeW; x++) setPx(x, y, EYE_ICE);
    for (let x = exR; x < exR + eyeW; x++) setPx(x, y, EYE_ICE);
  }
}
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = ICE_WHITE[0];
    canvas[i + 1] = ICE_WHITE[1];
    canvas[i + 2] = ICE_WHITE[2];
    canvas[i + 3] = 255;
  }
}
