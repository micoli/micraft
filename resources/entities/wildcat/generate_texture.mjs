// Builds wildcat.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "wildcat.bbmodel");

const SEED = 20260911 + 32;
...
// ---- Colors -----------------------------------------------------------------
const FELINE = [176, 158, 130];
const FELINE_DARK = [110, 94, 72];
const EYE_GREEN = [70, 170, 90];

// ---- Geometry: bas et musclé, oreilles à pinceaux, queue courte -----------------
const elements = [
  { name: "body", from: [-2, 6, -5], to: [2, 10, 4], bone: "body" },
  { name: "head", from: [-2, 7, -8], to: [2, 10, -5], bone: "head" },
  { name: "earL", from: [-2, 10, -7], to: [-1, 12, -6], bone: "head" },
  { name: "tuftL", from: [-2, 12, -7], to: [-1, 13, -6], bone: "head" },
  { name: "earR", from: [1, 10, -7], to: [2, 12, -6], bone: "head" },
  { name: "tuftR", from: [1, 12, -7], to: [2, 13, -6], bone: "head" },
  { name: "tail", from: [-1, 7, 4], to: [1, 8, 6], bone: "tail" },
  { name: "frontLegL", from: [-2, 0, -4], to: [-1, 6, -3], bone: "frontLegL" },
  { name: "frontLegR", from: [1, 0, -4], to: [2, 6, -3], bone: "frontLegR" },
  { name: "backLegL", from: [-2, 0, 2], to: [-1, 6, 3], bone: "backLegL" },
  { name: "backLegR", from: [1, 0, 2], to: [2, 6, 3], bone: "backLegR" },
];

const bones = {
  body: { origin: [0, 10, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 10, -5], rotation: [0, 0, 0] },
  tail: { origin: [0, 8, 4], rotation: [-15, 0, 0] },
  frontLegL: { origin: [-1.5, 6, -3.5], rotation: [0, 0, 0] },
  frontLegR: { origin: [1.5, 6, -3.5], rotation: [0, 0, 0] },
  backLegL: { origin: [-1.5, 6, 2.5], rotation: [0, 0, 0] },
  backLegR: { origin: [1.5, 6, 2.5], rotation: [0, 0, 0] },
};
...
function paintSpotted(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = FELINE;
      const mott = valueNoise(x, y, 2);
      if (mott > 0.68) base = FELINE_DARK;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "tuftL" || el.name === "tuftR") paintFlat(rect, FELINE_DARK, 4);
    else paintSpotted(rect);
  }
}
...
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  for (let y = ey; y < ey + eyeH; y++) {
    for (let x = exL; x < exL + eyeW; x++) setPx(x, y, EYE_GREEN);
    for (let x = exR; x < exR + eyeW; x++) setPx(x, y, EYE_GREEN);
  }
}
