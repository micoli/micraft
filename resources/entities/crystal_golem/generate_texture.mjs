// Builds crystal_golem.bbmodel geometry (deterministic) and paints its texture procedurally.
// Variant of golem: pale rock body with actual crystal spike geometry, blue glowing veins.
...
const OUT = join(__dirname, "crystal_golem.bbmodel");

const SEED = 20260911 + 47;
...
// ---- Colors -----------------------------------------------------------------
const STONE = [176, 178, 182];
const STONE_DARK = [140, 142, 148];
const CRYSTAL = [110, 190, 224];
const EYE_GLOW = [130, 210, 240];

// ---- Geometry: identique au golem, excroissances cristallines aux épaules -----
const elements = [
  { name: "body", from: [-5, 12, -3], to: [5, 26, 3], bone: "body" },
  { name: "head", from: [-3, 26, -3], to: [3, 31, 3], bone: "head" },
  { name: "crystalHead", from: [-1, 31, -1], to: [1, 34, 1], bone: "head" },
  { name: "rightArm", from: [-8, 12, -3], to: [-5, 24, 3], bone: "rightArm" },
  { name: "crystalShoulderR", from: [-9, 22, -1], to: [-8, 25, 1], bone: "rightArm" },
  { name: "leftArm", from: [5, 12, -3], to: [8, 24, 3], bone: "leftArm" },
  { name: "crystalShoulderL", from: [8, 22, -1], to: [9, 25, 1], bone: "leftArm" },
  { name: "rightLeg", from: [-4, 0, -3], to: [0, 12, 3], bone: "rightLeg" },
  { name: "leftLeg", from: [0, 0, -3], to: [4, 12, 3], bone: "leftLeg" },
];

const bones = {
  body: { origin: [0, 12, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 26, 0], rotation: [0, 0, 0] },
  rightArm: { origin: [-6.5, 24, 0], rotation: [0, 0, 0] },
  leftArm: { origin: [6.5, 24, 0], rotation: [0, 0, 0] },
  rightLeg: { origin: [-2, 12, 0], rotation: [0, 0, 0] },
  leftLeg: { origin: [2, 12, 0], rotation: [0, 0, 0] },
};
...
function paintRock(rect, isTorso) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = STONE;
      const grain = valueNoise(x, y, 1.5);
      if (grain > 0.55) base = STONE_DARK;
      let px = jitter(base, 10);
      if (isTorso) {
        const vein = valueNoise(x + 50, y + 50, 4);
        if (vein > 0.8) px = CRYSTAL;
      }
      setPx(x, y, px);
    }
  }
}
function paintFlat(rect, rgb, amount = 4) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) for (let x = x1; x < x2; x++) setPx(x, y, jitter(rgb, amount));
}

for (const el of builtElements) {
  const isCrystal = el.name.startsWith("crystal");
  for (const face of faceOrder) {
    const rect = el.faces[face].uv;
    if (isCrystal) paintFlat(rect, CRYSTAL, 5);
    else paintRock(rect, el.name === "body" || el.name === "rightArm" || el.name === "leftArm");
  }
}

for (const el of builtElements) {
  if (el.name.startsWith("crystal")) continue;
  for (const face of ["up", "down"]) {
    const [x1, y1, x2, y2] = el.faces[face].uv;
    const delta = face === "up" ? 8 : -10;
    for (let y = y1; y < y2; y++)
      for (let x = x1; x < x2; x++) {
        const i = (y * RES_W + x) * 4;
...
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  const eyeW = Math.max(1, Math.round(w * 0.2));
  const eyeH = Math.max(1, Math.round(h * 0.18));
  const ey = y1 + Math.round(h * 0.35);
  const exL = x1 + Math.round(w * 0.14);
  const exR = x2 - Math.round(w * 0.14) - eyeW;
  for (let y = ey; y < ey + eyeH; y++) {
    for (let x = exL; x < exL + eyeW; x++) setPx(x, y, EYE_GLOW);
    for (let x = exR; x < exR + eyeW; x++) setPx(x, y, EYE_GLOW);
  }
}
