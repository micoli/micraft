// Builds spider.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "spider.bbmodel");

const SEED = 20260911 + 5;
...
// ---- Colors -----------------------------------------------------------------
const CHITIN_BLACK = [28, 24, 22];
const CHITIN_BROWN = [58, 44, 34];
const LEG_LIGHT = [70, 58, 48];
const EYE_RED = [140, 22, 20];

// ---- Geometry: 2 segments (cephalothorax + abdomen), 8 thin legs ----------------
const elements = [
  { name: "cephalothorax", from: [-2, 3, -2], to: [2, 6, 2], bone: "cephalothorax" },
  { name: "abdomen", from: [-3, 3, 2], to: [3, 7, 7], bone: "cephalothorax" },
  { name: "leg0", from: [0, -4, 0], to: [1, 0, 1], bone: "leg0" },
  { name: "leg1", from: [0, -4, 0], to: [1, 0, 1], bone: "leg1" },
  { name: "leg2", from: [0, -4, 0], to: [1, 0, 1], bone: "leg2" },
  { name: "leg3", from: [0, -4, 0], to: [1, 0, 1], bone: "leg3" },
  { name: "leg4", from: [0, -4, 0], to: [1, 0, 1], bone: "leg4" },
  { name: "leg5", from: [0, -4, 0], to: [1, 0, 1], bone: "leg5" },
  { name: "leg6", from: [0, -4, 0], to: [1, 0, 1], bone: "leg6" },
  { name: "leg7", from: [0, -4, 0], to: [1, 0, 1], bone: "leg7" },
];

const bones = {
  cephalothorax: { origin: [0, 4.5, 0], rotation: [0, 0, 0] },
  leg0: { origin: [-2, 5, -1], rotation: [30, 0, -60] },
  leg1: { origin: [2, 5, -1], rotation: [30, 0, 60] },
  leg2: { origin: [-2, 5, 0], rotation: [20, 0, -80] },
  leg3: { origin: [2, 5, 0], rotation: [20, 0, 80] },
  leg4: { origin: [-2, 5, 1], rotation: [20, 0, -90] },
  leg5: { origin: [2, 5, 1], rotation: [20, 0, 90] },
  leg6: { origin: [-2, 5, 2], rotation: [10, 0, -100] },
  leg7: { origin: [2, 5, 2], rotation: [10, 0, 100] },
};
...
function paintAbdomen(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = CHITIN_BLACK;
      const mott = valueNoise(x, y, 2);
      if (mott > 0.55) base = CHITIN_BROWN;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name.startsWith("leg")) paintFlat(rect, LEG_LIGHT, 6);
    else if (el.name === "abdomen") paintAbdomen(rect);
    else paintFlat(rect, CHITIN_BLACK, 8);
  }
}

for (const el of builtElements) {
  if (el.name.startsWith("leg")) continue;
...
// Eye cluster painted last on cephalothorax's north (front) face
const headEl = builtElements.find((e) => e.name === "cephalothorax");
{
  const [x1, y1, x2, y2] = headEl.faces.north.uv;
  const w = x2 - x1, h = y2 - y1;
  const positions = [
    [0.2, 0.3], [0.5, 0.25], [0.8, 0.3],
    [0.35, 0.55], [0.65, 0.55],
  ];
  for (const [px, py] of positions) {
    const ex = x1 + Math.round(w * px);
    const ey = y1 + Math.round(h * py);
    setPx(ex, ey, EYE_RED);
  }
}
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = CHITIN_BLACK[0];
    canvas[i + 1] = CHITIN_BLACK[1];
    canvas[i + 2] = CHITIN_BLACK[2];
    canvas[i + 3] = 255;
  }
}
