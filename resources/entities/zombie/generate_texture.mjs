// Builds zombie.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "zombie.bbmodel");

const SEED = 20260911 + 11;
...
// ---- Colors -----------------------------------------------------------------
const SKIN = [112, 128, 96];
const SKIN_NECRO = [76, 90, 64];
const CLOTH = [92, 78, 56];
const CLOTH_DARK = [66, 56, 40];
const EYE_PALE = [214, 214, 202];

// ---- Geometry: proportions humanoïdes standards, posture raide -------------------
const elements = [
...
const bones = {
  ...
  head: { origin: [0, 24, 0], rotation: [5, 0, 0] },
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "body") paintCloth(rect);
    else paintSkin(rect);
  }
}
...
// Vacant pale eyes, painted last on head's north face
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  const exL = x1 + Math.round(w * 0.15);
  const exR = x2 - Math.round(w * 0.15) - eyeW;
  for (let y = ey; y < ey + eyeH; y++) {
    for (let x = exL; x < exL + eyeW; x++) setPx(x, y, EYE_PALE);
    for (let x = exR; x < exR + eyeW; x++) setPx(x, y, EYE_PALE);
  }
}
