// Builds vulture.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "vulture.bbmodel");

const SEED = 20260911 + 6;
...
// ---- Colors -----------------------------------------------------------------
const PLUMAGE_DARK = [50, 42, 36];
const PLUMAGE_BROWN = [70, 58, 46];
const NECK_SKIN = [156, 118, 106];
const BEAK_PALE = [198, 188, 158];
const EYE_DARK = [16, 12, 10];

// ---- Geometry: voûté/décharné, long cou nu, grandes ailes en cape ----------------
const elements = [
  { name: "body", from: [-3, 6, -4], to: [3, 13, 5], bone: "body" },
  { name: "neck", from: [-1, 13, -3], to: [1, 16, -1], bone: "neck" },
  { name: "head", from: [-2, 16, -4], to: [2, 19, -1], bone: "head" },
  { name: "beak", from: [-1, 17, -5], to: [1, 18, -4], bone: "head" },
  { name: "wingL", from: [-4, 7, -3], to: [-3, 13, 4], bone: "wingL" },
  { name: "wingR", from: [3, 7, -3], to: [4, 13, 4], bone: "wingR" },
...
const bones = {
  ...
  neck: { origin: [0, 13, -2], rotation: [20, 0, 0] },
  head: { origin: [0, 16, -2.5], rotation: [0, 0, 0] },
...
function paintPlumage(rect) {
  ...
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = PLUMAGE_DARK;
      const mott = valueNoise(x, y, 2.5);
      if (mott > 0.6) base = PLUMAGE_BROWN;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "beak") paintFlat(rect, BEAK_PALE, 4);
    else if (el.name === "neck" || el.name === "head") paintFlat(rect, NECK_SKIN, 7);
    else if (el.name === "legL" || el.name === "legR") paintFlat(rect, BEAK_PALE, 6);
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = PLUMAGE_DARK[0];
    canvas[i + 1] = PLUMAGE_DARK[1];
    canvas[i + 2] = PLUMAGE_DARK[2];
    canvas[i + 3] = 255;
  }
}
