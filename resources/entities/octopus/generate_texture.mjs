// Builds octopus.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "octopus.bbmodel");

const SEED = 20260911 + 17;
...
// ---- Colors -----------------------------------------------------------------
const MANTLE = [122, 62, 46];
const MANTLE_SPOT = [90, 44, 32];
const TENTACLE = [104, 52, 38];
const EYE_DARK = [14, 10, 8];

// ---- Geometry: manteau bulbeux + 8 tentacules pendantes ------------------------
const elements = [
  { name: "mantle", from: [-3, 6, -3], to: [3, 10, 3], bone: "mantle" },
  { name: "t0", from: [0, -6, 0], to: [1, 0, 1], bone: "t0" },
  { name: "t1", from: [0, -6, 0], to: [1, 0, 1], bone: "t1" },
  { name: "t2", from: [0, -6, 0], to: [1, 0, 1], bone: "t2" },
  { name: "t3", from: [0, -6, 0], to: [1, 0, 1], bone: "t3" },
  { name: "t4", from: [0, -6, 0], to: [1, 0, 1], bone: "t4" },
  { name: "t5", from: [0, -6, 0], to: [1, 0, 1], bone: "t5" },
  { name: "t6", from: [0, -6, 0], to: [1, 0, 1], bone: "t6" },
  { name: "t7", from: [0, -6, 0], to: [1, 0, 1], bone: "t7" },
];

const bones = {
  mantle: { origin: [0, 8, 0], rotation: [0, 0, 0] },
  t0: { origin: [-2, 6, -2], rotation: [0, 0, -15] },
  t1: { origin: [2, 6, -2], rotation: [0, 0, 15] },
  t2: { origin: [-3, 6, -1], rotation: [0, 0, -20] },
  t3: { origin: [3, 6, -1], rotation: [0, 0, 20] },
  t4: { origin: [-3, 6, 1], rotation: [0, 0, -20] },
  t5: { origin: [3, 6, 1], rotation: [0, 0, 20] },
  t6: { origin: [-2, 6, 2], rotation: [0, 0, -15] },
  t7: { origin: [2, 6, 2], rotation: [0, 0, 15] },
};
...
function paintMantle(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = MANTLE;
      const mott = valueNoise(x, y, 2);
      if (mott > 0.65) base = MANTLE_SPOT;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "mantle") paintMantle(rect);
    else paintFlat(rect, TENTACLE, 6);
  }
}

for (const el of builtElements) {
  if (el.name !== "mantle") continue;
...
const mantleEl = builtElements.find((e) => e.name === "mantle");
{
  const [x1, y1, x2, y2] = mantleEl.faces.north.uv;
