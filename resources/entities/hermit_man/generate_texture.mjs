// Builds hermit_man.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "hermit_man.bbmodel");

const SEED = 20260911 + 7;
...
// ---- Colors -----------------------------------------------------------------
const CLOTH = [150, 126, 92];
const CLOTH_DARK = [118, 98, 68];
const HOOD = [108, 88, 62];
const SKIN = [196, 158, 122];
const BEARD = [182, 180, 174];
const EYE_DARK = [24, 20, 16];

// ---- Geometry: proportions humaines standards, capuche + barbe, posture voûtée -----
const elements = [
  { name: "body", from: [-4, 12, -2], to: [4, 24, 2], bone: "body" },
  { name: "head", from: [-4, 24, -4], to: [4, 32, 4], bone: "head" },
  { name: "hood", from: [-5, 29, -5], to: [5, 34, 5], bone: "head" },
  { name: "beard", from: [-2, 21, -5], to: [2, 25, -4], bone: "head" },
...
const bones = {
  body: { origin: [0, 12, 0], rotation: [8, 0, 0] },
  head: { origin: [0, 24, 0], rotation: [8, 0, 0] },
...
function paintPatchwork(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = CLOTH;
      const mott = valueNoise(x, y, 4);
      if (mott > 0.55) base = CLOTH_DARK;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "hood") paintFlat(rect, HOOD, 7);
    else if (el.name === "beard") paintFlat(rect, BEARD, 5);
    else if (el.name === "head") paintFlat(rect, SKIN, 6);
    else paintPatchwork(rect);
  }
}
...
// Eyes painted last on head's north face, above the beard line
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  const ey = y1 + Math.round(h * 0.4);
  const exL = x1 + Math.round(w * 0.2);
  const exR = x2 - Math.round(w * 0.2) - eyeW;
