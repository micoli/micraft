// Builds frost_wyrm.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "frost_wyrm.bbmodel");

const SEED = 20260911 + 38;
...
// ---- Colors -----------------------------------------------------------------
const SCALE = [104, 156, 198];
const SCALE_DARK = [72, 120, 166];
const VEIN = [222, 240, 250];
const EYE_GLOW = [220, 246, 255];

// ---- Geometry: dragon serpentiforme, ailes réduites, cornes recourbées ---------
const elements = [
  { name: "head", from: [-3, 10, -14], to: [3, 15, -8], bone: "head" },
  { name: "jaw", from: [-2, 9, -15], to: [2, 10, -14], bone: "head" },
  { name: "hornL", from: [-2, 15, -12], to: [-1, 17, -11], bone: "head" },
  { name: "hornR", from: [1, 15, -12], to: [2, 17, -11], bone: "head" },
  { name: "seg1", from: [-2, 10, -8], to: [2, 14, -2], bone: "seg1" },
  { name: "seg2", from: [-2, 10, -2], to: [2, 13, 4], bone: "seg2" },
  { name: "seg3", from: [-1, 10, 4], to: [1, 12, 10], bone: "seg3" },
  { name: "wingL", from: [-6, 12, -6], to: [-3, 15, 0], bone: "wingL" },
  { name: "wingR", from: [3, 12, -6], to: [6, 15, 0], bone: "wingR" },
];

const bones = {
  head: { origin: [0, 12, -8], rotation: [0, 0, 0] },
  seg1: { origin: [0, 12, -8], rotation: [0, 0, 0] },
  seg2: { origin: [0, 11, -2], rotation: [0, 0, 0] },
  seg3: { origin: [0, 11, 4], rotation: [0, 0, 0] },
  wingL: { origin: [-3, 13, -3], rotation: [0, 0, 0] },
  wingR: { origin: [3, 13, -3], rotation: [0, 0, 0] },
};
...
function paintScales(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = SCALE;
      const mott = valueNoise(x, y, 1.8);
      if (mott > 0.6) base = SCALE_DARK;
      if (mott > 0.85) base = VEIN;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "jaw" || el.name === "hornL" || el.name === "hornR") paintFlat(rect, VEIN, 4);
    else paintScales(rect);
  }
}
