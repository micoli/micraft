// Builds hedgehog.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "hedgehog.bbmodel");

const SEED = 20260911 + 26;
...
// ---- Colors -----------------------------------------------------------------
const BELLY = [198, 180, 150];
const SPIKE = [72, 60, 48];
const SPIKE_DARK = [50, 42, 34];
const NOSE = [18, 16, 14];

// ---- Geometry: minuscule, moitié basse lisse, moitié haute hérissée ------------
const elements = [
  { name: "body", from: [-1, 1, -2], to: [1, 3, 2], bone: "body" },
  { name: "spikeBack", from: [-1, 3, -1], to: [1, 4, 2], bone: "body" },
  { name: "nose", from: [0, 1, -3], to: [1, 2, -2], bone: "body" },
  { name: "frontLegL", from: [-1, 0, -1], to: [0, 1, 0], bone: "body" },
  { name: "frontLegR", from: [0, 0, -1], to: [1, 1, 0], bone: "body" },
];

const bones = { body: { origin: [0, 2, 0], rotation: [0, 0, 0] } };
...
function paintSpikes(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      const base = (x + y) % 2 === 0 ? SPIKE : SPIKE_DARK;
      setPx(x, y, jitter(base, 5));
    }
  }
}
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "spikeBack") paintSpikes(rect);
    else if (el.name === "nose") paintFlat(rect, NOSE, 2);
    else paintFlat(rect, BELLY, 5);
  }
}
...
const bodyEl = builtElements.find((e) => e.name === "body");
{
  ...
  const w = x2 - x1;
  const y = y1;
  setPx(x1 + Math.max(0, Math.round(w * 0.3)), y, NOSE);
  setPx(x1 + Math.max(0, Math.round(w * 0.7)), y, NOSE);
}
