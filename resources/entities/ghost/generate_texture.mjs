// Builds ghost.bbmodel geometry (deterministic) and paints its texture procedurally.
// Note: the texture pipeline requires a fully opaque canvas (no real alpha transparency),
// so the "translucent" look is approximated with a pale blue-white palette only.
...
const OUT = join(__dirname, "ghost.bbmodel");

const SEED = 20260911 + 15;
...
// ---- Colors -----------------------------------------------------------------
const GHOST_BASE = [198, 216, 226];
const GHOST_DARK = [162, 184, 198];
const EYE_HOLLOW = [22, 32, 36];

// ---- Geometry: humanoid sans jambes distinctes, voile effilé, bras fins ---------
const elements = [
...
function paintMist(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = GHOST_BASE;
      const mott = valueNoise(x, y, 3.5);
      if (mott > 0.55) base = GHOST_DARK;
...
for (const el of builtElements) {
  for (const face of faceOrder) paintMist(el.faces[face].uv);
}
...
// Hollow eyes painted last on head's north face
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = GHOST_BASE[0];
    canvas[i + 1] = GHOST_BASE[1];
    canvas[i + 2] = GHOST_BASE[2];
    canvas[i + 3] = 255;
  }
}
