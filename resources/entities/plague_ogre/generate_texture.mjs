// Builds plague_ogre.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "plague_ogre.bbmodel");

const SEED = 20260911 + 46;
...
// ---- Colors -----------------------------------------------------------------
const SKIN = [92, 110, 70];
const SKIN_DARK = [62, 78, 46];
const PUSTULE = [148, 74, 62];
const EYE_SICK = [192, 182, 52];

// ---- Geometry: proportions asymétriques, un bras plus gros, dos bossu ---------
const elements = [
  { name: "body", from: [-4, 10, -3], to: [4, 20, 4], bone: "body" },
  { name: "head", from: [-3, 20, -3], to: [3, 25, 3], bone: "head" },
  { name: "rightArm", from: [-8, 10, -3], to: [-4, 20, 3], bone: "rightArm" },
  { name: "leftArm", from: [4, 12, -2], to: [6, 20, 2], bone: "leftArm" },
  { name: "rightLeg", from: [-4, 0, -3], to: [0, 10, 3], bone: "rightLeg" },
  { name: "leftLeg", from: [0, 0, -3], to: [4, 10, 3], bone: "leftLeg" },
];

const bones = {
  body: { origin: [0, 10, 0], rotation: [8, 0, 0] },
  head: { origin: [0, 20, 0], rotation: [8, 0, 0] },
  rightArm: { origin: [-6, 20, 0], rotation: [0, 0, 0] },
  leftArm: { origin: [5, 20, 0], rotation: [0, 0, 0] },
...
function paintDiseased(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = SKIN;
      const mott = valueNoise(x, y, 2);
      if (mott > 0.62) base = SKIN_DARK;
      if (mott > 0.86) base = PUSTULE;
      setPx(x, y, jitter(base, 11));
    }
  }
}

for (const el of builtElements) {
  for (const face of faceOrder) paintDiseased(el.faces[face].uv);
}
...
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  for (let y = ey; y < ey + eyeH; y++) {
    for (let x = exL; x < exL + eyeW; x++) setPx(x, y, EYE_SICK);
    for (let x = exR; x < exR + eyeW; x++) setPx(x, y, EYE_SICK);
  }
}
