// Builds pharaoh_wraith.bbmodel geometry (deterministic) and paints its texture procedurally.
// Reskin of the ghost silhouette with a golden funerary mask and bandaged torso.
...
const OUT = join(__dirname, "pharaoh_wraith.bbmodel");

const SEED = 20260911 + 40;
...
// ---- Colors -----------------------------------------------------------------
const SHROUD = [124, 96, 150];
const SHROUD_DARK = [96, 72, 120];
const GOLD = [214, 174, 76];
const EYE_HOLLOW = [30, 20, 40];

// ---- Geometry: façon ghost mais orné, masque doré, voile drapé -----------------
const elements = [
  { name: "body", from: [-3, 10, -2], to: [3, 20, 2], bone: "body" },
  { name: "head", from: [-3, 20, -3], to: [3, 26, 3], bone: "head" },
  { name: "mask", from: [-3, 20, -4], to: [3, 26, -3], bone: "head" },
  { name: "shroudTop", from: [-3, 4, -2], to: [3, 10, 2], bone: "shroud" },
  { name: "shroudBottom", from: [-2, 0, -2], to: [2, 4, 2], bone: "shroud" },
  { name: "rightArm", from: [-5, 14, -1], to: [-3, 20, 1], bone: "rightArm" },
  { name: "leftArm", from: [3, 14, -1], to: [5, 20, 1], bone: "leftArm" },
];

const bones = {
  body: { origin: [0, 10, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 20, 0], rotation: [0, 0, 0] },
  shroud: { origin: [0, 4, 0], rotation: [0, 0, 0] },
  rightArm: { origin: [-3, 20, 0], rotation: [0, 0, 0] },
  leftArm: { origin: [3, 20, 0], rotation: [0, 0, 0] },
};
...
function paintShroud(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = SHROUD;
      const mott = valueNoise(x, y, 3);
      if (mott > 0.55) base = SHROUD_DARK;
      setPx(x, y, jitter(base, 7));
    }
  }
}
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "mask") paintFlat(rect, GOLD, 5);
    else paintShroud(rect);
  }
}

for (const el of builtElements) {
  if (el.name === "mask") continue;
  for (const face of ["up", "down"]) {
    const [x1, y1, x2, y2] = el.faces[face].uv;
    const delta = face === "up" ? 6 : -10;
    for (let y = y1; y < y2; y++)
      for (let x = x1; x < x2; x++) {
        const i = (y * RES_W + x) * 4;
...
// Hollow dark eyes painted last on the mask's own face
const maskEl = builtElements.find((e) => e.name === "mask");
{
  const [x1, y1, x2, y2] = maskEl.faces.north.uv;
  ...
  for (let y = ey; y < ey + eyeH; y++) {
    for (let x = exL; x < exL + eyeW; x++) setPx(x, y, EYE_HOLLOW);
    for (let x = exR; x < exR + eyeW; x++) setPx(x, y, EYE_HOLLOW);
  }
}
