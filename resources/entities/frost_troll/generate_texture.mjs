// Builds frost_troll.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "frost_troll.bbmodel");

const SEED = 20260911 + 36;
...
// ---- Colors -----------------------------------------------------------------
const SKIN = [92, 110, 118];
const SKIN_DARK = [64, 82, 90];
const TUSK = [222, 218, 202];
const EYE_DARK = [16, 14, 12];

// ---- Geometry: massif, peau nue grumeleuse, défenses, bras très longs -----------
const elements = [
  { name: "body", from: [-5, 13, -3], to: [5, 25, 3], bone: "body" },
  { name: "head", from: [-3, 25, -3], to: [3, 30, 3], bone: "head" },
  { name: "tuskL", from: [-2, 25, -4], to: [-1, 27, -3], bone: "head" },
  { name: "tuskR", from: [1, 25, -4], to: [2, 27, -3], bone: "head" },
  { name: "rightArm", from: [-9, 10, -3], to: [-5, 25, 3], bone: "rightArm" },
  { name: "leftArm", from: [5, 10, -3], to: [9, 25, 3], bone: "leftArm" },
  { name: "rightLeg", from: [-5, 0, -3], to: [0, 13, 3], bone: "rightLeg" },
  { name: "leftLeg", from: [0, 0, -3], to: [5, 13, 3], bone: "leftLeg" },
];

const bones = {
  body: { origin: [0, 13, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 25, 0], rotation: [10, 0, 0] },
  rightArm: { origin: [-7, 25, 0], rotation: [0, 0, 0] },
  leftArm: { origin: [7, 25, 0], rotation: [0, 0, 0] },
  rightLeg: { origin: [-2.5, 13, 0], rotation: [0, 0, 0] },
  leftLeg: { origin: [2.5, 13, 0], rotation: [0, 0, 0] },
};
...
function paintWartySkin(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = SKIN;
      const mott = valueNoise(x, y, 1.5);
      if (mott > 0.6) base = SKIN_DARK;
      setPx(x, y, jitter(base, 14));
    }
  }
}
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "tuskL" || el.name === "tuskR") paintFlat(rect, TUSK, 3);
    else paintWartySkin(rect);
  }
}

for (const el of builtElements) {
  if (el.name.startsWith("tusk")) continue;
