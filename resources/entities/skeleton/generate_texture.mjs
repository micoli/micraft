// Builds skeleton.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "skeleton.bbmodel");

const SEED = 20260911 + 10;
...
// ---- Colors -----------------------------------------------------------------
const BONE = [214, 208, 196];
const BONE_SHADE = [172, 164, 150];
const EYE_SOCKET = [12, 10, 8];

// ---- Geometry: humanoid amaigri, membres fins, pas de vêtements ------------------
const elements = [
  { name: "body", from: [-3, 12, -1], to: [3, 24, 1], bone: "body" },
  { name: "head", from: [-3, 24, -3], to: [3, 30, 3], bone: "head" },
  { name: "rightArm", from: [-6, 12, -1], to: [-3, 24, 1], bone: "rightArm" },
  { name: "leftArm", from: [3, 12, -1], to: [6, 24, 1], bone: "leftArm" },
  { name: "rightLeg", from: [-3, 0, -1], to: [0, 12, 1], bone: "rightLeg" },
  { name: "leftLeg", from: [0, 0, -1], to: [3, 12, 1], bone: "leftLeg" },
];

const bones = {
  body: { origin: [0, 12, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 24, 0], rotation: [0, 0, 0] },
  rightArm: { origin: [-3, 24, 0], rotation: [0, 0, 0] },
  leftArm: { origin: [3, 24, 0], rotation: [0, 0, 0] },
  rightLeg: { origin: [-1.5, 12, 0], rotation: [0, 0, 0] },
  leftLeg: { origin: [1.5, 12, 0], rotation: [0, 0, 0] },
};
...
function paintBone(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = BONE;
      const mott = valueNoise(x, y, 3);
      if (mott > 0.7) base = BONE_SHADE;
...
for (const el of builtElements) {
  for (const face of faceOrder) paintBone(el.faces[face].uv);
}

// Joint shading: darken the down/up faces of limbs for a "socket" look
for (const el of builtElements) {
  if (el.name === "body" || el.name === "head") continue;
  for (const face of ["up", "down"]) {
    const [x1, y1, x2, y2] = el.faces[face].uv;
    for (let y = y1; y < y2; y++)
      for (let x = x1; x < x2; x++) {
        const i = (y * RES_W + x) * 4;
        canvas[i] = Math.max(0, canvas[i] - 20);
        canvas[i + 1] = Math.max(0, canvas[i + 1] - 20);
        canvas[i + 2] = Math.max(0, canvas[i + 2] - 20);
      }
  }
}

// Hollow eye sockets painted last on head's north face
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  for (let y = ey; y < ey + eyeH; y++) {
    for (let x = exL; x < exL + eyeW; x++) setPx(x, y, EYE_SOCKET);
    for (let x = exR; x < exR + eyeW; x++) setPx(x, y, EYE_SOCKET);
  }
}
