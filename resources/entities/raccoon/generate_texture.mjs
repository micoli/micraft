// Builds raccoon.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "raccoon.bbmodel");

const SEED = 20260911 + 25;
...
// ---- Colors -----------------------------------------------------------------
const FUR = [112, 106, 98];
const FUR_DARK = [72, 66, 60];
const MASK = [30, 26, 24];
const RING_LIGHT = [200, 194, 184];
const EYE_DARK = [16, 14, 12];

// ---- Geometry: trapu, masque facial, queue annelée ------------------------------
const elements = [
  { name: "body", from: [-2, 4, -4], to: [2, 9, 3], bone: "body" },
  { name: "head", from: [-2, 9, -6], to: [2, 13, -4], bone: "head" },
  { name: "mask", from: [-2, 9, -7], to: [2, 11, -6], bone: "head" },
  { name: "tailRing0", from: [-1, 6, 3], to: [1, 8, 5], bone: "tail" },
  { name: "tailRing1", from: [-1, 6, 5], to: [1, 8, 7], bone: "tail" },
  { name: "tailRing2", from: [-1, 6, 7], to: [1, 8, 9], bone: "tail" },
  { name: "leg0", from: [-1, 0, -2], to: [0, 4, 0], bone: "leg0" },
  { name: "leg1", from: [0, 0, -2], to: [1, 4, 0], bone: "leg1" },
];

const bones = {
  body: { origin: [0, 8, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 9, -4], rotation: [0, 0, 0] },
  tail: { origin: [0, 7, 3], rotation: [-10, 0, 0] },
  leg0: { origin: [-0.5, 4, -1], rotation: [0, 0, 0] },
  leg1: { origin: [0.5, 4, -1], rotation: [0, 0, 0] },
};
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "mask") paintFlat(rect, MASK, 4);
    else if (el.name === "tailRing0" || el.name === "tailRing2") paintFlat(rect, FUR_DARK, 5);
    else if (el.name === "tailRing1") paintFlat(rect, RING_LIGHT, 5);
    else paintFlat(rect, FUR, 7);
  }
}

for (const el of builtElements) {
  if (el.name.startsWith("tailRing")) continue;
...
// Eyes painted last on head's north face, within the black mask
