// Builds kraken_spawn.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "kraken_spawn.bbmodel");

const SEED = 20260911 + 41;
...
// ---- Colors -----------------------------------------------------------------
const HEAD = [40, 60, 54];
const HEAD_GLOW = [60, 150, 140];
const TENTACLE = [48, 70, 62];
const EYE_DARK = [12, 10, 10];

// ---- Geometry: tête bulbeuse à œil unique, couronne de tentacules épaisses -----
const elements = [
  { name: "head", from: [-4, 8, -4], to: [4, 14, 4], bone: "head" },
...
const bones = {
  head: { origin: [0, 11, 0], rotation: [0, 0, 0] },
  t0: { origin: [-3, 8, -3], rotation: [0, 0, -15] },
  t1: { origin: [3, 8, -3], rotation: [0, 0, 15] },
  t2: { origin: [-4, 8, -1], rotation: [0, 0, -20] },
  t3: { origin: [4, 8, -1], rotation: [0, 0, 20] },
  t4: { origin: [-4, 8, 1], rotation: [0, 0, -20] },
  t5: { origin: [4, 8, 1], rotation: [0, 0, 20] },
  t6: { origin: [-3, 8, 3], rotation: [0, 0, -15] },
  t7: { origin: [3, 8, 3], rotation: [0, 0, 15] },
};
...
function paintHead(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = HEAD;
      const mott = valueNoise(x, y, 2);
      if (mott > 0.75) base = HEAD_GLOW;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "head") paintHead(rect);
    else paintFlat(rect, TENTACLE, 6);
  }
}

for (const el of builtElements) {
  if (el.name !== "head") continue;
...
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  const ex = x1 + Math.round((w - eyeW) / 2);
  const ey = y1 + Math.round(h * 0.3);
  for (let y = ey; y < ey + eyeH; y++)
    for (let x = ex; x < ex + eyeW; x++) setPx(x, y, EYE_DARK);
}
