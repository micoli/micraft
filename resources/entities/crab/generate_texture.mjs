// Builds crab.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "crab.bbmodel");

const SEED = 20260911 + 30;
...
// ---- Colors -----------------------------------------------------------------
const SHELL = [196, 92, 52];
const SHELL_DARK = [154, 68, 36];
const CLAW_TIP = [226, 150, 100];
const EYE_DARK = [14, 10, 8];

// ---- Geometry: carapace ovale plate, pinces asymétriques, pattes en éventail -----
const elements = [
  { name: "shell", from: [-3, 1, -2], to: [3, 3, 2], bone: "shell" },
  { name: "pincerL", from: [-5, 1, -1], to: [-3, 3, 1], bone: "pincerL" },
  { name: "pincerR", from: [3, 1, -1], to: [4, 2, 1], bone: "pincerR" },
  { name: "leg0", from: [0, -1, 0], to: [1, 1, 1], bone: "leg0" },
  { name: "leg1", from: [0, -1, 0], to: [1, 1, 1], bone: "leg1" },
  { name: "leg2", from: [0, -1, 0], to: [1, 1, 1], bone: "leg2" },
  { name: "leg3", from: [0, -1, 0], to: [1, 1, 1], bone: "leg3" },
  { name: "leg4", from: [0, -1, 0], to: [1, 1, 1], bone: "leg4" },
  { name: "leg5", from: [0, -1, 0], to: [1, 1, 1], bone: "leg5" },
];

const bones = {
  shell: { origin: [0, 2, 0], rotation: [0, 0, 0] },
  pincerL: { origin: [-3, 2, 0], rotation: [0, 0, -20] },
  pincerR: { origin: [3, 2, 0], rotation: [0, 0, 20] },
  leg0: { origin: [-3, 1, -1], rotation: [10, 0, -70] },
  leg1: { origin: [3, 1, -1], rotation: [10, 0, 70] },
  leg2: { origin: [-3, 1, 0], rotation: [10, 0, -90] },
  leg3: { origin: [3, 1, 0], rotation: [10, 0, 90] },
  leg4: { origin: [-3, 1, 1], rotation: [10, 0, -110] },
  leg5: { origin: [3, 1, 1], rotation: [10, 0, 110] },
};
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "pincerL" || el.name === "pincerR") paintFlat(rect, CLAW_TIP, 5);
    else if (el.name.startsWith("leg")) paintFlat(rect, SHELL_DARK, 5);
    else paintFlat(rect, SHELL, 7);
  }
}
...
const shellEl = builtElements.find((e) => e.name === "shell");
{
  const [x1, y1, x2, y2] = shellEl.faces.north.uv;
  const w = x2 - x1;
  const y = y1;
  setPx(x1 + Math.max(0, Math.round(w * 0.3)), y, EYE_DARK);
  setPx(x1 + Math.max(0, Math.round(w * 0.7)), y, EYE_DARK);
}
