// Builds bog_hydra.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "bog_hydra.bbmodel");

const SEED = 20260911 + 44;
...
// ---- Colors -----------------------------------------------------------------
const SKIN = [70, 84, 52];
const SKIN_DARK = [48, 58, 36];
const EYE_YELLOW = [204, 192, 40];

// ---- Geometry: corps quadrupède trapu + trois cous/têtes de serpent -----------
const elements = [
  { name: "body", from: [-3, 3, -4], to: [3, 7, 4], bone: "body" },
  { name: "neck0", from: [-1, 7, -4], to: [1, 9, -2], bone: "neck0" },
  { name: "head0", from: [-1, 9, -5], to: [1, 11, -3], bone: "neck0" },
  { name: "neck1", from: [-1, 7, -1], to: [1, 10, 1], bone: "neck1" },
  { name: "head1", from: [-1, 10, -1], to: [1, 12, 1], bone: "neck1" },
  { name: "neck2", from: [-1, 7, 2], to: [1, 9, 4], bone: "neck2" },
  { name: "head2", from: [-1, 9, 3], to: [1, 11, 5], bone: "neck2" },
  { name: "frontLegL", from: [-3, 0, -3], to: [-1, 3, -1], bone: "frontLegL" },
  { name: "frontLegR", from: [1, 0, -3], to: [3, 3, -1], bone: "frontLegR" },
  { name: "backLegL", from: [-3, 0, 1], to: [-1, 3, 3], bone: "backLegL" },
  { name: "backLegR", from: [1, 0, 1], to: [3, 3, 3], bone: "backLegR" },
];

const bones = {
  body: { origin: [0, 5, 0], rotation: [0, 0, 0] },
  neck0: { origin: [0, 7, -4], rotation: [-20, -20, 0] },
  neck1: { origin: [0, 7, 0], rotation: [-10, 0, 0] },
  neck2: { origin: [0, 7, 4], rotation: [-20, 20, 0] },
  frontLegL: { origin: [-2, 3, -2], rotation: [0, 0, 0] },
  frontLegR: { origin: [2, 3, -2], rotation: [0, 0, 0] },
  backLegL: { origin: [-2, 3, 2], rotation: [0, 0, 0] },
  backLegR: { origin: [2, 3, 2], rotation: [0, 0, 0] },
};
...
function paintSkin(rect, dark) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = dark ? SKIN_DARK : SKIN;
      const mott = valueNoise(x, y, 2);
      if (mott > 0.65) base = SKIN_DARK;
...
for (const el of builtElements) {
  const isHead = el.name.startsWith("head");
  for (const face of faceOrder) paintSkin(el.faces[face].uv, isHead);
}
...
for (const headName of ["head0", "head1", "head2"]) {
  const el = builtElements.find((e) => e.name === headName);
  const [x1, y1, x2, y2] = el.faces.north.uv;
  const w = x2 - x1;
  const y = y1;
  setPx(x1 + Math.max(0, Math.round(w * 0.3)), y, EYE_YELLOW);
  setPx(x1 + Math.max(0, Math.round(w * 0.7)), y, EYE_YELLOW);
}
