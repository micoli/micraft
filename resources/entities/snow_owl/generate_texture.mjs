// Builds snow_owl.bbmodel geometry (deterministic) and paints its texture procedurally.
// Reskin of the owl silhouette in a white/black-spot palette.
...
const OUT = join(__dirname, "snow_owl.bbmodel");

const SEED = 20260911 + 14;
...
// ---- Colors -----------------------------------------------------------------
const PLUMAGE_WHITE = [236, 236, 232];
const SPOT_BLACK = [40, 38, 36];
const EYE_YELLOW = [230, 200, 40];
const BEAK = [46, 36, 24];
const LEG_COLOR = [220, 214, 200];

// ---- Geometry: identique à owl (trapu, ailes repliées) ---------------------------
const elements = [
...
function paintSpotted(rect) {
  ...
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = PLUMAGE_WHITE;
      const mott = valueNoise(x, y, 2.5);
      if (mott > 0.82) base = SPOT_BLACK;
      setPx(x, y, jitter(base, 6));
    }
  }
}
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    else if (el.name === "legL" || el.name === "legR") paintFlat(rect, LEG_COLOR, 5);
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = PLUMAGE_WHITE[0];
    canvas[i + 1] = PLUMAGE_WHITE[1];
    canvas[i + 2] = PLUMAGE_WHITE[2];
    canvas[i + 3] = 255;
  }
}
