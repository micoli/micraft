// Builds ancient_treant.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "ancient_treant.bbmodel");

const SEED = 20260911 + 39;
...
// ---- Colors -----------------------------------------------------------------
const BARK = [74, 54, 38];
const BARK_DARK = [50, 36, 24];
const MOSS = [86, 112, 56];
const EYE_GLOW = [96, 208, 118];

// ---- Geometry: tronc noueux, branches en bras, racines en jambes ---------------
const elements = [
  { name: "body", from: [-4, 10, -3], to: [4, 22, 3], bone: "body" },
  { name: "head", from: [-3, 22, -3], to: [3, 27, 3], bone: "head" },
  { name: "mossPatch", from: [-3, 27, -3], to: [3, 28, 3], bone: "head" },
  { name: "rightArm", from: [-7, 14, -2], to: [-4, 22, 2], bone: "rightArm" },
  { name: "leftArm", from: [4, 14, -2], to: [7, 22, 2], bone: "leftArm" },
  { name: "rightLeg", from: [-4, 0, -3], to: [0, 10, 3], bone: "rightLeg" },
  { name: "leftLeg", from: [0, 0, -3], to: [4, 10, 3], bone: "leftLeg" },
];

const bones = {
  body: { origin: [0, 10, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 22, 0], rotation: [0, 0, 0] },
  rightArm: { origin: [-4, 22, 0], rotation: [0, 0, 0] },
  leftArm: { origin: [4, 22, 0], rotation: [0, 0, 0] },
  rightLeg: { origin: [-2, 10, 0], rotation: [0, 0, 0] },
  leftLeg: { origin: [2, 10, 0], rotation: [0, 0, 0] },
};
...
function paintBark(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = BARK;
      const mott = valueNoise(x, y, 1.3);
      if (mott > 0.55) base = BARK_DARK;
      setPx(x, y, jitter(base, 12));
    }
  }
}
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "mossPatch") paintFlat(rect, MOSS, 6);
    else paintBark(rect);
  }
}

for (const el of builtElements) {
  if (el.name === "mossPatch") continue;
