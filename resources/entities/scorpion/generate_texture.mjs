// Builds scorpion.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "scorpion.bbmodel");

const SEED = 20260911 + 8;
...
// ---- Colors -----------------------------------------------------------------
const CARAPACE = [96, 40, 28];
const CARAPACE_DARK = [66, 26, 18];
const TAIL_TIP = [30, 24, 20];
const EYE_DARK = [10, 8, 6];

// ---- Geometry: corps plat, pinces, queue recourbée segmentée, 8 pattes ----------
const elements = [
  { name: "body", from: [-2, 1, -3], to: [2, 3, 3], bone: "body" },
  { name: "pincerL", from: [-3, 1, -4], to: [-1, 2, -3], bone: "body" },
  { name: "pincerR", from: [1, 1, -4], to: [3, 2, -3], bone: "body" },
  { name: "tailSeg1", from: [-1, 2, 3], to: [1, 3, 5], bone: "tail" },
  { name: "tailSeg2", from: [-1, 3, 5], to: [1, 4, 7], bone: "tail" },
  { name: "stinger", from: [-1, 4, 7], to: [1, 5, 8], bone: "tail" },
  { name: "leg0", from: [0, -2, 0], to: [1, 0, 1], bone: "leg0" },
  { name: "leg1", from: [0, -2, 0], to: [1, 0, 1], bone: "leg1" },
  { name: "leg2", from: [0, -2, 0], to: [1, 0, 1], bone: "leg2" },
  { name: "leg3", from: [0, -2, 0], to: [1, 0, 1], bone: "leg3" },
  { name: "leg4", from: [0, -2, 0], to: [1, 0, 1], bone: "leg4" },
  { name: "leg5", from: [0, -2, 0], to: [1, 0, 1], bone: "leg5" },
  { name: "leg6", from: [0, -2, 0], to: [1, 0, 1], bone: "leg6" },
  { name: "leg7", from: [0, -2, 0], to: [1, 0, 1], bone: "leg7" },
];

const bones = {
  body: { origin: [0, 2, 0], rotation: [0, 0, 0] },
  tail: { origin: [0, 2, 3], rotation: [-40, 0, 0] },
  leg0: { origin: [-2, 2, -2], rotation: [10, 0, -60] },
  leg1: { origin: [2, 2, -2], rotation: [10, 0, 60] },
  leg2: { origin: [-2, 2, -1], rotation: [10, 0, -80] },
  leg3: { origin: [2, 2, -1], rotation: [10, 0, 80] },
  leg4: { origin: [-2, 2, 0], rotation: [10, 0, -90] },
  leg5: { origin: [2, 2, 0], rotation: [10, 0, 90] },
  leg6: { origin: [-2, 2, 1], rotation: [10, 0, -100] },
  leg7: { origin: [2, 2, 1], rotation: [10, 0, 100] },
};
...
function paintCarapace(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = CARAPACE;
      const mott = valueNoise(x, y, 2);
      if (mott > 0.6) base = CARAPACE_DARK;
      setPx(x, y, jitter(base, 8));
    }
  }
}
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "stinger") paintFlat(rect, TAIL_TIP, 4);
    else if (el.name.startsWith("leg")) paintFlat(rect, CARAPACE_DARK, 6);
    else paintCarapace(rect);
  }
}

for (const el of builtElements) {
  if (el.name.startsWith("leg") || el.name === "stinger") continue;
...
// Tiny eye dots on body's north (front) face, painted last
const bodyEl = builtElements.find((e) => e.name === "body");
{
  const [x1, y1, x2, y2] = bodyEl.faces.north.uv;
  const w = x2 - x1;
  const y = y1;
  setPx(x1 + Math.round(w * 0.3), y, EYE_DARK);
  setPx(x1 + Math.round(w * 0.7), y, EYE_DARK);
}
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = CARAPACE[0];
    canvas[i + 1] = CARAPACE[1];
    canvas[i + 2] = CARAPACE[2];
    canvas[i + 3] = 255;
  }
}
