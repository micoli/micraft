// Builds magma_golem.bbmodel geometry (deterministic) and paints its texture procedurally.
// Variant of golem: basalt body, wider/brighter lava cracks.
...
const OUT = join(__dirname, "magma_golem.bbmodel");

const SEED = 20260911 + 43;
...
// ---- Colors -----------------------------------------------------------------
const BASALT = [42, 38, 36];
const BASALT_DARK = [26, 24, 22];
const LAVA = [238, 108, 32];
const EYE_GLOW = [250, 150, 40];

// ---- Geometry: identique au golem, proportions cubiques marquées --------------
const elements = [
...
function paintRock(rect, isTorso) {
  ...
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      ...
      if (grain > 0.55) base = BASALT_DARK;
      let px = jitter(base, 12);
      if (isTorso) {
        const crack = valueNoise(x + 50, y + 50, 3.5);
        if (crack > 0.72) px = LAVA;
      }
      setPx(x, y, px);
    }
  }
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    paintRock(el.faces[face].uv, el.name === "body" || el.name === "rightArm" || el.name === "leftArm");
  }
}
