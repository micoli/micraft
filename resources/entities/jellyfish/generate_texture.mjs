// Builds jellyfish.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "jellyfish.bbmodel");

const SEED = 20260911 + 18;
...
// ---- Colors -----------------------------------------------------------------
const DOME = [214, 184, 214];
const DOME_SHADE = [186, 156, 190];
const TENTACLE = [198, 170, 198];
const GLOW = [238, 224, 238];

// ---- Geometry: dôme hémisphérique + rideau de tentacules fins -------------------
const elements = [
  { name: "dome", from: [-2, 6, -2], to: [2, 9, 2], bone: "dome" },
...
const bones = {
  dome: { origin: [0, 7.5, 0], rotation: [0, 0, 0] },
  t0: { origin: [-1, 6, -1], rotation: [0, 0, -8] },
  t1: { origin: [1, 6, -1], rotation: [0, 0, 8] },
  t2: { origin: [-2, 6, 0], rotation: [0, 0, -10] },
  t3: { origin: [2, 6, 0], rotation: [0, 0, 10] },
  t4: { origin: [-2, 6, 1], rotation: [0, 0, -10] },
  t5: { origin: [2, 6, 1], rotation: [0, 0, 10] },
  t6: { origin: [-1, 6, 2], rotation: [0, 0, -8] },
  t7: { origin: [1, 6, 2], rotation: [0, 0, 8] },
};
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "dome") paintFlat(rect, face === "down" ? DOME_SHADE : DOME, 6);
    else paintFlat(rect, TENTACLE, 5);
  }
}

// Faint bioluminescent glow at the center of the dome's front face, painted last
const domeEl = builtElements.find((e) => e.name === "dome");
{
  const [x1, y1, x2, y2] = domeEl.faces.north.uv;
  const w = x2 - x1, h = y2 - y1;
  const gx = x1 + Math.round(w / 2);
  const gy = y1 + Math.round(h / 2);
  setPx(gx, gy, GLOW);
}
