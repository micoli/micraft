// Builds desert_raider_man.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "desert_raider_man.bbmodel");

const SEED = 20260911 + 35;
...
// ---- Colors -----------------------------------------------------------------
const SAND_CLOTH = [196, 168, 120];
const SAND_CLOTH_DARK = [164, 138, 96];
const LEATHER = [70, 56, 42];
const SKIN = [188, 150, 112];
const EYE_DARK = [22, 18, 14];

// ---- Geometry: proportions humanoïdes standards + turban + plastron + bandages --
const elements = [
  { name: "body", from: [-4, 12, -2], to: [4, 24, 2], bone: "body" },
  { name: "head", from: [-4, 24, -4], to: [4, 32, 4], bone: "head" },
  { name: "turban", from: [-4, 29, -4], to: [4, 33, 4], bone: "head" },
  { name: "plastron", from: [-4, 15, -3], to: [4, 22, 3], bone: "body" },
  { name: "rightArm", from: [-8, 12, -2], to: [-4, 24, 2], bone: "rightArm" },
  { name: "leftArm", from: [4, 12, -2], to: [8, 24, 2], bone: "leftArm" },
  { name: "bandageR", from: [-8, 12, -2], to: [-4, 16, 2], bone: "rightArm" },
  { name: "bandageL", from: [4, 12, -2], to: [8, 16, 2], bone: "leftArm" },
  { name: "rightLeg", from: [-4, 0, -2], to: [0, 12, 2], bone: "rightLeg" },
  { name: "leftLeg", from: [0, 0, -2], to: [4, 12, 2], bone: "leftLeg" },
];

const bones = {
  body: { origin: [0, 12, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 24, 0], rotation: [0, 0, 0] },
  rightArm: { origin: [-4, 24, 0], rotation: [0, 0, 0] },
  leftArm: { origin: [4, 24, 0], rotation: [0, 0, 0] },
...
function paintCloth(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = SAND_CLOTH;
      const mott = valueNoise(x, y, 3.5);
      if (mott > 0.6) base = SAND_CLOTH_DARK;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "plastron") paintFlat(rect, LEATHER, 5);
    else if (el.name === "turban" || el.name === "bandageR" || el.name === "bandageL")
      paintFlat(rect, SAND_CLOTH_DARK, 5);
    else if (el.name === "head") paintFlat(rect, SKIN, 6);
    else paintCloth(rect);
  }
}
...
// Eyes above the turban's face wrap, painted last on head's north face
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  const eyeH = Math.max(1, Math.round(h * 0.12));
  const ey = y1 + Math.round(h * 0.25);
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = SAND_CLOTH[0];
    canvas[i + 1] = SAND_CLOTH[1];
    canvas[i + 2] = SAND_CLOTH[2];
    canvas[i + 3] = 255;
  }
}
