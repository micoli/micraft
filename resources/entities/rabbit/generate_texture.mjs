// Builds rabbit.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "rabbit.bbmodel");

const SEED = 20260911 + 1;
...
// ---- Colors -----------------------------------------------------------------
const FUR_GRAYBROWN = [150, 130, 108];
const FUR_GRAYBROWN_DARK = [122, 104, 84];
const BELLY_WHITE = [240, 234, 224];
const EYE_BLACK = [18, 16, 14];

// ---- Geometry -----------------------------------------------------------------
const elements = [
  { name: "body", from: [-2, 4, -3], to: [2, 8, 3], bone: "body" },
  { name: "head", from: [-2, 6, -6], to: [2, 10, -3], bone: "head" },
  { name: "earL", from: [-1, 10, -5], to: [0, 14, -4], bone: "head" },
  { name: "earR", from: [0, 10, -5], to: [1, 14, -4], bone: "head" },
  { name: "tail", from: [-1, 5, 3], to: [1, 7, 4], bone: "tail" },
  { name: "frontLegL", from: [-1, 0, -2], to: [0, 4, -1], bone: "frontLegL" },
  { name: "frontLegR", from: [0, 0, -2], to: [1, 4, -1], bone: "frontLegR" },
  { name: "backLegL", from: [-2, 0, 1], to: [-1, 5, 3], bone: "backLegL" },
  { name: "backLegR", from: [1, 0, 1], to: [2, 5, 3], bone: "backLegR" },
];

const bones = {
  body: { origin: [0, 8, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 10, -3], rotation: [0, 0, 0] },
  tail: { origin: [0, 6, 3.5], rotation: [-20, 0, 0] },
  frontLegL: { origin: [-0.5, 4, -1.5], rotation: [0, 0, 0] },
  frontLegR: { origin: [0.5, 4, -1.5], rotation: [0, 0, 0] },
  backLegL: { origin: [-1.5, 5, 2], rotation: [0, 0, 0] },
  backLegR: { origin: [1.5, 5, 2], rotation: [0, 0, 0] },
};
...
function paintFurRect(rect, belly) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = FUR_GRAYBROWN;
      if (belly) {
        const t = (y - y1) / Math.max(1, y2 - y1 - 1);
        base = base.map((c, i) => Math.round(c * (1 - t) + BELLY_WHITE[i] * t));
      }
      const mott = valueNoise(x, y, 3);
      if (mott > 0.6) base = base.map((c, i) => Math.round(c * 0.85 + FUR_GRAYBROWN_DARK[i] * 0.15));
      setPx(x, y, jitter(base, 8));
    }
  }
}
function paintPompom(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) for (let x = x1; x < x2; x++) setPx(x, y, jitter(BELLY_WHITE, 6));
}

for (const el of builtElements) {
  const isBelly = el.name === "body";
  const isTail = el.name === "tail";
  for (const face of faceOrder) {
    const rect = el.faces[face].uv;
    if (isTail) paintPompom(rect);
    else paintFurRect(rect, isBelly && face === "down");
  }
}

for (const el of builtElements) {
  if (el.name === "tail") continue;
...
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  for (let y = ey; y < ey + eyeH; y++) {
    for (let x = exL; x < exL + eyeW; x++) setPx(x, y, EYE_BLACK);
    for (let x = exR; x < exR + eyeW; x++) setPx(x, y, EYE_BLACK);
  }
}
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = FUR_GRAYBROWN[0];
    canvas[i + 1] = FUR_GRAYBROWN[1];
    canvas[i + 2] = FUR_GRAYBROWN[2];
    canvas[i + 3] = 255;
  }
}
