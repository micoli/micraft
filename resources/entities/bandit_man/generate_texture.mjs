// Builds bandit_man.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "bandit_man.bbmodel");

const SEED = 20260911 + 12;
...
// ---- Colors -----------------------------------------------------------------
const LEATHER = [56, 46, 36];
const LEATHER_DARK = [40, 32, 24];
const SKIN = [196, 160, 124];
const BANDANA = [148, 128, 94];
const EYE_DARK = [22, 18, 14];

// ---- Geometry: proportions humanoïdes standards + bandana + épaulière asymétrique -
const elements = [
  { name: "body", from: [-4, 12, -2], to: [4, 24, 2], bone: "body" },
  { name: "head", from: [-4, 24, -4], to: [4, 32, 4], bone: "head" },
  { name: "bandana", from: [-4, 24, -5], to: [4, 27, -4], bone: "head" },
  { name: "rightArm", from: [-8, 12, -2], to: [-4, 24, 2], bone: "rightArm" },
  { name: "leftArm", from: [4, 12, -2], to: [8, 24, 2], bone: "leftArm" },
  { name: "shoulderPad", from: [-9, 20, -3], to: [-8, 24, 3], bone: "rightArm" },
...
function paintLeather(rect) {
  ...
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      ...
      if (mott > 0.62) base = LEATHER_DARK;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "bandana") paintFlat(rect, BANDANA, 6);
    else if (el.name === "head") paintFlat(rect, SKIN, 7);
    else if (el.name === "shoulderPad") paintFlat(rect, LEATHER_DARK, 5);
    else paintLeather(rect);
  }
}
...
// Narrow eyes above the bandana, painted last on head's north face
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  const eyeH = Math.max(1, Math.round(h * 0.1));
  const ey = y1 + Math.round(h * 0.2);
  const exL = x1 + Math.round(w * 0.18);
  const exR = x2 - Math.round(w * 0.18) - eyeW;
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = LEATHER[0];
    canvas[i + 1] = LEATHER[1];
    canvas[i + 2] = LEATHER[2];
    canvas[i + 3] = 255;
  }
}
