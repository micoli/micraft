// Builds mole.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "mole.bbmodel");

const SEED = 20260911 + 24;
...
// ---- Colors -----------------------------------------------------------------
const FUR = [50, 46, 44];
const NOSE = [214, 148, 148];
const EYE_DARK = [18, 16, 14];

// ---- Geometry: cylindrique trapu, pattes-pelles avant démesurées ----------------
const elements = [
  { name: "body", from: [-2, 1, -3], to: [2, 3, 3], bone: "body" },
  { name: "nose", from: [-1, 1, -4], to: [1, 2, -3], bone: "body" },
  { name: "frontPawL", from: [-3, 0, -2], to: [-1, 1, 0], bone: "frontPawL" },
  { name: "frontPawR", from: [1, 0, -2], to: [3, 1, 0], bone: "frontPawR" },
  { name: "backLeg", from: [-1, 0, 2], to: [1, 1, 3], bone: "backLeg" },
];

const bones = {
  body: { origin: [0, 2, 0], rotation: [0, 0, 0] },
  frontPawL: { origin: [-2, 1, -1], rotation: [0, 0, 0] },
  frontPawR: { origin: [2, 1, -1], rotation: [0, 0, 0] },
  backLeg: { origin: [0, 1, 2.5], rotation: [0, 0, 0] },
};
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "nose") paintFlat(rect, NOSE, 3);
    else paintFlat(rect, FUR, 4);
  }
}
