// Builds moose.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "moose.bbmodel");

const SEED = 20260911 + 31;
...
// ---- Colors -----------------------------------------------------------------
const FUR = [62, 48, 36];
const FUR_DARK = [44, 34, 24];
const ANTLER = [214, 204, 182];
const EYE_DARK = [16, 14, 12];

// ---- Geometry: le plus grand gabarit hors boss, panache large, museau tombant ----
const elements = [
  { name: "body", from: [-4, 14, -8], to: [4, 22, 6], bone: "body" },
  { name: "head", from: [-3, 22, -11], to: [3, 26, -8], bone: "head" },
  { name: "muzzle", from: [-2, 20, -13], to: [2, 22, -11], bone: "head" },
  { name: "antlerL1", from: [-6, 26, -10], to: [-3, 27, -8], bone: "head" },
  { name: "antlerL2", from: [-8, 26, -9], to: [-6, 27, -6], bone: "head" },
  { name: "antlerR1", from: [3, 26, -10], to: [6, 27, -8], bone: "head" },
  { name: "antlerR2", from: [6, 26, -9], to: [8, 27, -6], bone: "head" },
  { name: "frontLegL", from: [-3, 0, -6], to: [-1, 14, -4], bone: "frontLegL" },
  { name: "frontLegR", from: [1, 0, -6], to: [3, 14, -4], bone: "frontLegR" },
  { name: "backLegL", from: [-3, 0, 3], to: [-1, 14, 5], bone: "backLegL" },
  { name: "backLegR", from: [1, 0, 3], to: [3, 14, 5], bone: "backLegR" },
];

const bones = {
  body: { origin: [0, 22, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 22, -8], rotation: [0, 0, 0] },
  frontLegL: { origin: [-2, 14, -5], rotation: [0, 0, 0] },
  frontLegR: { origin: [2, 14, -5], rotation: [0, 0, 0] },
  backLegL: { origin: [-2, 14, 4], rotation: [0, 0, 0] },
  backLegR: { origin: [2, 14, 4], rotation: [0, 0, 0] },
};
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name.startsWith("antler")) paintFlat(rect, ANTLER, 4);
    else paintFur(rect);
  }
}

for (const el of builtElements) {
  if (el.name.startsWith("antler")) continue;
