// Builds giant_toad.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "giant_toad.bbmodel");

const SEED = 20260911 + 37;
...
// ---- Colors -----------------------------------------------------------------
const GREEN = [92, 120, 54];
const GREEN_DARK = [62, 86, 36];
const BELLY_PALE = [214, 204, 140];
const EYE_DARK = [16, 14, 10];

// ---- Geometry: bulbeux, yeux protubérants, pattes arrière puissantes -----------
const elements = [
  { name: "body", from: [-3, 1, -4], to: [3, 6, 4], bone: "body" },
  { name: "eyeBumpL", from: [-2, 6, -2], to: [-1, 7, -1], bone: "body" },
  { name: "eyeBumpR", from: [1, 6, -2], to: [2, 7, -1], bone: "body" },
  { name: "frontLegL", from: [-2, 0, -3], to: [-1, 1, -2], bone: "frontLegL" },
  { name: "frontLegR", from: [1, 0, -3], to: [2, 1, -2], bone: "frontLegR" },
  { name: "backLegL", from: [-3, 0, 2], to: [-1, 3, 4], bone: "backLegL" },
  { name: "backLegR", from: [1, 0, 2], to: [3, 3, 4], bone: "backLegR" },
];

const bones = {
  body: { origin: [0, 4, 0], rotation: [0, 0, 0] },
  frontLegL: { origin: [-1.5, 1, -2.5], rotation: [0, 0, 0] },
  frontLegR: { origin: [1.5, 1, -2.5], rotation: [0, 0, 0] },
  backLegL: { origin: [-2, 3, 3], rotation: [0, 0, 0] },
  backLegR: { origin: [2, 3, 3], rotation: [0, 0, 0] },
};
...
function paintWarty(rect, belly) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = belly ? BELLY_PALE : GREEN;
      const mott = valueNoise(x, y, 2.2);
      if (!belly && mott > 0.62) base = GREEN_DARK;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name.startsWith("eyeBump") && face !== "up") paintFlat(rect, EYE_DARK, 3);
    else paintWarty(rect, el.name === "body" && face === "down");
  }
}

for (const el of builtElements) {
  if (el.name.startsWith("eyeBump")) continue;
