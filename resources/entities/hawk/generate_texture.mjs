// Builds hawk.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "hawk.bbmodel");

const SEED = 20260911 + 23;
...
// ---- Colors -----------------------------------------------------------------
const RUFOUS = [138, 78, 46];
const RUFOUS_DARK = [104, 56, 32];
const CHEST_STRIPE = [214, 196, 168];
const BEAK = [46, 36, 24];
const EYE_YELLOW = [232, 202, 40];

// ---- Geometry: silhouette fuselée, tête plus petite que l'owl, ailes pointues ----
const elements = [
  { name: "body", from: [-2, 6, -4], to: [2, 12, 4], bone: "body" },
  { name: "head", from: [-2, 12, -4], to: [2, 16, -1], bone: "head" },
  { name: "beak", from: [-1, 12, -6], to: [1, 14, -4], bone: "head" },
  { name: "wingL", from: [-4, 8, -2], to: [-2, 12, 3], bone: "wingL" },
  { name: "wingR", from: [2, 8, -2], to: [4, 12, 3], bone: "wingR" },
  { name: "legL", from: [-1, 0, -1], to: [0, 6, 1], bone: "legL" },
  { name: "legR", from: [0, 0, -1], to: [1, 6, 1], bone: "legR" },
];

const bones = {
  body: { origin: [0, 6, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 12, -1], rotation: [0, 0, 0] },
  wingL: { origin: [-2, 12, 0], rotation: [0, 0, 0] },
  wingR: { origin: [2, 12, 0], rotation: [0, 0, 0] },
  legL: { origin: [-0.5, 6, 0], rotation: [0, 0, 0] },
  legR: { origin: [0.5, 6, 0], rotation: [0, 0, 0] },
};
...
function paintStriped(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      const base = (y - y1) % 2 === 0 ? CHEST_STRIPE : RUFOUS;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    else if (el.name === "body" && face === "down") paintStriped(rect);
    else paintFlat(rect, RUFOUS, 8);
  }
}
...
for (const el of builtElements) {
  ...
  for (const face of ["up", "down"]) {
    if (el.name === "body" && face === "down") continue;
...
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  const eyeW = Math.max(1, Math.round(w * 0.22));
  const eyeH = Math.max(1, Math.round(h * 0.22));
