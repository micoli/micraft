// Builds stone_lizard.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "stone_lizard.bbmodel");

const SEED = 20260911 + 28;
...
// ---- Colors -----------------------------------------------------------------
const SKIN = [140, 124, 96];
const SKIN_DARK = [104, 90, 68];
const EYE_DARK = [20, 18, 14];

// ---- Geometry: bas et allongé, longue queue traînante ---------------------------
const elements = [
  { name: "body", from: [-1, 1, -4], to: [1, 3, 4], bone: "body" },
  { name: "head", from: [-1, 1, -6], to: [1, 3, -4], bone: "head" },
  { name: "tail", from: [-1, 1, 4], to: [1, 2, 10], bone: "tail" },
  { name: "frontLegL", from: [-2, 0, -3], to: [-1, 1, -2], bone: "frontLegL" },
  { name: "frontLegR", from: [1, 0, -3], to: [2, 1, -2], bone: "frontLegR" },
  { name: "backLegL", from: [-2, 0, 2], to: [-1, 1, 3], bone: "backLegL" },
  { name: "backLegR", from: [1, 0, 2], to: [2, 1, 3], bone: "backLegR" },
];

const bones = {
  body: { origin: [0, 2, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 2, -4], rotation: [0, 0, 0] },
  tail: { origin: [0, 1.5, 4], rotation: [0, 0, 0] },
  frontLegL: { origin: [-1.5, 1, -2.5], rotation: [0, 0, 0] },
  frontLegR: { origin: [1.5, 1, -2.5], rotation: [0, 0, 0] },
  backLegL: { origin: [-1.5, 1, 2.5], rotation: [0, 0, 0] },
  backLegR: { origin: [1.5, 1, 2.5], rotation: [0, 0, 0] },
};
...
function paintScales(rect) {
  ...
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = SKIN;
      const scale = (Math.floor(x / 1) + Math.floor(y / 1)) % 2 === 0 ? 1 : 0;
      const mott = valueNoise(x, y, 2);
      if (mott > 0.6 || scale === 0) base = SKIN_DARK;
...
for (const el of builtElements) {
  for (const face of faceOrder) paintScales(el.faces[face].uv);
}
