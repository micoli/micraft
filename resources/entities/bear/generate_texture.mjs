// Builds bear.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "bear.bbmodel");

const SEED = 20260911 + 9;
...
// ---- Colors -----------------------------------------------------------------
const FUR_BROWN = [70, 52, 38];
const FUR_DARK = [48, 36, 26];
const MUZZLE = [112, 90, 68];
const EYE_DARK = [15, 12, 10];

// ---- Geometry: massif, museau court, pattes épaisses, petites oreilles rondes -----
const elements = [
  { name: "body", from: [-4, 7, -7], to: [4, 15, 7], bone: "body" },
  { name: "head", from: [-4, 9, -11], to: [4, 15, -7], bone: "head" },
  { name: "muzzle", from: [-2, 9, -13], to: [2, 12, -11], bone: "head" },
  { name: "earL", from: [-3, 15, -10], to: [-2, 17, -9], bone: "head" },
  { name: "earR", from: [2, 15, -10], to: [3, 17, -9], bone: "head" },
  { name: "tail", from: [-1, 10, 7], to: [1, 11, 8], bone: "body" },
  { name: "frontLegL", from: [-3, 0, -5], to: [-1, 7, -3], bone: "frontLegL" },
  { name: "frontLegR", from: [1, 0, -5], to: [3, 7, -3], bone: "frontLegR" },
  { name: "backLegL", from: [-3, 0, 4], to: [-1, 7, 6], bone: "backLegL" },
  { name: "backLegR", from: [1, 0, 4], to: [3, 7, 6], bone: "backLegR" },
];

const bones = {
  body: { origin: [0, 15, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 15, -7], rotation: [0, 0, 0] },
  frontLegL: { origin: [-2, 7, -4], rotation: [0, 0, 0] },
  frontLegR: { origin: [2, 7, -4], rotation: [0, 0, 0] },
  backLegL: { origin: [-2, 7, 5], rotation: [0, 0, 0] },
  backLegR: { origin: [2, 7, 5], rotation: [0, 0, 0] },
};
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "muzzle") paintFlat(rect, MUZZLE, 6);
    else paintFur(rect);
  }
}
...
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  const eyeH = Math.max(1, Math.round(h * 0.16));
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = FUR_BROWN[0];
    canvas[i + 1] = FUR_BROWN[1];
    canvas[i + 2] = FUR_BROWN[2];
    canvas[i + 3] = 255;
  }
}
