// Regenerates resources/entities/blacksmith/blacksmith.bbmodel — geometry (bones + cuboids with
...
// variant, then `make dc CMD="node resources/entities/blacksmith/generate_texture.mjs"`.
//
// blacksmith: a stocky humanoid village smith — grey tunic/trousers/rolled sleeves, long black
// leather apron (bib + skirt to the shins), thick brown beard and hair, heavy dark boots.
// interactionable, does not wander.
...
const BBMODEL = join(HERE, "blacksmith.bbmodel");

// ---------------------------------------------------------------------------- constants
const SEED = 0x424c4b53; // "BLKS"
const ATLAS_W = 64;

const COLORS = {
  cloth: { base: [120, 122, 126], accent: [95, 97, 102], grain: 7, mix: 0.32 },
  skin: { base: [214, 166, 130], accent: [227, 183, 150], grain: 6, mix: 0.24 },
  hair: { base: [74, 54, 38], accent: [52, 37, 25], grain: 10, mix: 0.42 },
  apron: { base: [37, 37, 41], accent: [23, 23, 27], grain: 5, mix: 0.35 },
  boot: { base: [45, 38, 33], accent: [29, 24, 21], grain: 6, mix: 0.35 },
};
const EYE_IRIS = [92, 58, 34]; // brown
...
const ELEMENTS = [
  { name: "body", from: [-4, 12, -2], to: [4, 24, 2], skin: "cloth" },
  { name: "head", from: [-4, 24, -4], to: [4, 32, 4], skin: "skin" },
  { name: "hair", from: [-4, 30, -4], to: [4, 33, 4], skin: "hair" },
  { name: "beard", from: [-3, 24, -5], to: [3, 29, -3], skin: "hair" },
  { name: "earR", from: [-5, 26, -1], to: [-4, 29, 1], skin: "skin" },
  { name: "earL", from: [4, 26, -1], to: [5, 29, 1], skin: "skin" },
  { name: "rightArm", from: [-8, 12, -2], to: [-4, 24, 2], skin: "cloth" },
  { name: "leftArm", from: [4, 12, -2], to: [8, 24, 2], skin: "cloth" },
  { name: "rightHand", from: [-8, 9, -2], to: [-4, 12, 2], skin: "skin" },
  { name: "leftHand", from: [4, 9, -2], to: [8, 12, 2], skin: "skin" },
  { name: "rightLeg", from: [-4, 0, -2], to: [0, 12, 2], skin: "cloth" },
  { name: "leftLeg", from: [0, 0, -2], to: [4, 12, 2], skin: "cloth" },
  { name: "apronBib", from: [-3, 13, -3], to: [3, 24, -2], skin: "apron" },
  { name: "apronSkirt", from: [-4, 2, -3], to: [4, 13, -2], skin: "apron" },
  { name: "rightBoot", from: [-4, 0, -3], to: [0, 3, 2], skin: "boot" },
  { name: "leftBoot", from: [0, 0, -3], to: [4, 3, 2], skin: "boot" },
];

const BONES = [
  { name: "body", origin: [0, 12, 0], elems: ["body", "apronBib", "apronSkirt"] },
  { name: "head", origin: [0, 24, 0], elems: ["head", "hair", "beard", "earR", "earL"] },
  { name: "rightArm", origin: [-4, 24, 0], elems: ["rightArm", "rightHand"] },
  { name: "leftArm", origin: [4, 24, 0], elems: ["leftArm", "leftHand"] },
  { name: "rightLeg", origin: [-2, 12, 0], elems: ["rightLeg", "rightBoot"] },
  { name: "leftLeg", origin: [2, 12, 0], elems: ["leftLeg", "leftBoot"] },
];
...
for (let i = 0; i < px.length; i += 4) {
  if (px[i + 3] !== 255) {
    px[i] = COLORS.cloth.base[0];
    px[i + 1] = COLORS.cloth.base[1];
    px[i + 2] = COLORS.cloth.base[2];
    px[i + 3] = 255;
  }
}
