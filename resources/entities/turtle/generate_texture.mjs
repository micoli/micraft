// Builds turtle.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "turtle.bbmodel");

const SEED = 20260911 + 19;
...
// ---- Colors -----------------------------------------------------------------
const SHELL = [92, 110, 58];
const SHELL_DARK = [64, 80, 38];
const SKIN = [140, 154, 116];
const EYE_DARK = [16, 14, 10];

// ---- Geometry: trapu, carapace haute, pattes-nageoires courtes ------------------
const elements = [
  { name: "body", from: [-3, 3, -4], to: [3, 6, 4], bone: "body" },
  { name: "shell", from: [-4, 6, -5], to: [4, 10, 5], bone: "body" },
  { name: "head", from: [-2, 3, -6], to: [2, 5, -4], bone: "head" },
...
function paintShell(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = SHELL;
      // grille de plaques via un motif géométrique répétitif
      const plate = (Math.floor(x / 3) + Math.floor(y / 3)) % 2 === 0;
      if (plate) base = SHELL_DARK;
      setPx(x, y, jitter(base, 6));
    }
  }
}
function paintSkin(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++)
    for (let x = x1; x < x2; x++) setPx(x, y, jitter(SKIN, 7));
}

for (const el of builtElements) {
  for (const face of faceOrder) {
    const rect = el.faces[face].uv;
    if (el.name === "shell") paintShell(rect);
    else paintSkin(rect);
  }
}

for (const el of builtElements) {
  if (el.name === "shell") continue;
  for (const face of ["up", "down"]) {
    const [x1, y1, x2, y2] = el.faces[face].uv;
    const delta = face === "up" ? 6 : -8;
    for (let y = y1; y < y2; y++)
      for (let x = x1; x < x2; x++) {
        const i = (y * RES_W + x) * 4;
