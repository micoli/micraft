// Builds yeti.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "yeti.bbmodel");

const SEED = 20260911 + 20;
...
// ---- Colors -----------------------------------------------------------------
const FUR_WHITE = [232, 232, 226];
const FUR_SHADE = [198, 200, 194];
const SKIN = [92, 100, 110];
const EYE_DARK = [18, 16, 14];

// ---- Geometry: massif, simiesque, bras longs, torse et épaules très larges ------
const elements = [
  { name: "body", from: [-6, 14, -3], to: [6, 28, 3], bone: "body" },
  { name: "head", from: [-4, 28, -4], to: [4, 34, 4], bone: "head" },
  { name: "rightArm", from: [-10, 10, -3], to: [-6, 28, 3], bone: "rightArm" },
  { name: "leftArm", from: [6, 10, -3], to: [10, 28, 3], bone: "leftArm" },
  { name: "rightLeg", from: [-6, 0, -3], to: [0, 14, 3], bone: "rightLeg" },
  { name: "leftLeg", from: [0, 0, -3], to: [6, 14, 3], bone: "leftLeg" },
];

const bones = {
  body: { origin: [0, 14, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 28, 0], rotation: [0, 0, 0] },
  rightArm: { origin: [-8, 28, 0], rotation: [0, 0, 0] },
  leftArm: { origin: [8, 28, 0], rotation: [0, 0, 0] },
  rightLeg: { origin: [-3, 14, 0], rotation: [0, 0, 0] },
  leftLeg: { origin: [3, 14, 0], rotation: [0, 0, 0] },
};
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "head" && (face === "north" || face === "south" || face === "east" || face === "west"))
      paintFlat(rect, SKIN, 7);
    else paintFur(rect);
  }
}
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = FUR_WHITE[0];
    canvas[i + 1] = FUR_WHITE[1];
    canvas[i + 2] = FUR_WHITE[2];
    canvas[i + 3] = 255;
  }
}
