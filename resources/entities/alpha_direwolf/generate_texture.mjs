// Builds alpha_direwolf.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "alpha_direwolf.bbmodel");

const SEED = 20260911 + 42;
...
// ---- Colors -----------------------------------------------------------------
const FUR = [54, 50, 48];
const FUR_DARK = [36, 32, 30];
const MANE = [96, 88, 80];
const EYE_RED = [214, 90, 40];

// ---- Geometry: loup géant (échelle ~1.4x), crinière épaisse, cicatrices --------
const elements = [
  { name: "body", from: [-4, 9, -9], to: [4, 16, 8], bone: "body" },
  { name: "mane", from: [-4, 15, -9], to: [4, 18, -2], bone: "body" },
  { name: "head", from: [-4, 11, -14], to: [4, 16, -9], bone: "head" },
  { name: "snout", from: [-2, 11, -16], to: [2, 13, -14], bone: "head" },
  { name: "earL", from: [-3, 16, -13], to: [-2, 19, -11], bone: "head" },
  { name: "earR", from: [2, 16, -13], to: [3, 19, -11], bone: "head" },
  { name: "tail", from: [-2, 10, 8], to: [2, 13, 15], bone: "tail" },
  { name: "frontLegL", from: [-3, 0, -7], to: [-1, 9, -5], bone: "frontLegL" },
  { name: "frontLegR", from: [1, 0, -7], to: [3, 9, -5], bone: "frontLegR" },
  { name: "backLegL", from: [-3, 0, 4], to: [-1, 9, 6], bone: "backLegL" },
  { name: "backLegR", from: [1, 0, 4], to: [3, 9, 6], bone: "backLegR" },
];

const bones = {
  body: { origin: [0, 16, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 16, -9], rotation: [0, 0, 0] },
  tail: { origin: [0, 13, 8], rotation: [-15, 0, 0] },
  frontLegL: { origin: [-2, 9, -6], rotation: [0, 0, 0] },
  frontLegR: { origin: [2, 9, -6], rotation: [0, 0, 0] },
  backLegL: { origin: [-2, 9, 5], rotation: [0, 0, 0] },
  backLegR: { origin: [2, 9, 5], rotation: [0, 0, 0] },
};
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "mane") paintFlat(rect, MANE, 8);
    else paintFur(rect);
  }
}

for (const el of builtElements) {
  if (el.name === "mane") continue;
...
const headEl = builtElements.find((e) => e.name === "head");
{
  ...
  for (let y = ey; y < ey + eyeH; y++) {
    for (let x = exL; x < exL + eyeW; x++) setPx(x, y, EYE_RED);
    for (let x = exR; x < exR + eyeW; x++) setPx(x, y, EYE_RED);
  }
}
