// Builds boar.bbmodel geometry (deterministic) and paints its texture procedurally.
...
const OUT = join(__dirname, "boar.bbmodel");

const SEED = 20260911 + 3;
...
// ---- Colors -----------------------------------------------------------------
const HIDE_BROWN = [82, 70, 58];
const HIDE_DARK = [56, 48, 40];
const CREST = [34, 30, 26];
const TUSK = [232, 226, 210];
const EYE_DARK = [18, 14, 10];

// ---- Geometry: robuste, bas sur pattes, crête, défenses --------------------------
const elements = [
  { name: "body", from: [-3, 5, -6], to: [3, 11, 6], bone: "body" },
  { name: "head", from: [-3, 7, -10], to: [3, 12, -6], bone: "head" },
  { name: "crest", from: [-1, 11, -3], to: [1, 13, 3], bone: "body" },
  { name: "tuskL", from: [-2, 8, -11], to: [-1, 9, -10], bone: "head" },
  { name: "tuskR", from: [1, 8, -11], to: [2, 9, -10], bone: "head" },
  { name: "tail", from: [-1, 9, 6], to: [1, 10, 7], bone: "body" },
  { name: "frontLegL", from: [-2, 0, -4], to: [0, 5, -2], bone: "frontLegL" },
  { name: "frontLegR", from: [0, 0, -4], to: [2, 5, -2], bone: "frontLegR" },
  { name: "backLegL", from: [-2, 0, 3], to: [0, 5, 5], bone: "backLegL" },
  { name: "backLegR", from: [0, 0, 3], to: [2, 5, 5], bone: "backLegR" },
];

const bones = {
  body: { origin: [0, 11, 0], rotation: [0, 0, 0] },
  head: { origin: [0, 11, -6], rotation: [0, 0, 0] },
  frontLegL: { origin: [-1, 5, -3], rotation: [0, 0, 0] },
  frontLegR: { origin: [1, 5, -3], rotation: [0, 0, 0] },
  backLegL: { origin: [-1, 5, 4], rotation: [0, 0, 0] },
  backLegR: { origin: [1, 5, 4], rotation: [0, 0, 0] },
};
...
function paintHide(rect) {
  const [x1, y1, x2, y2] = rect;
  for (let y = y1; y < y2; y++) {
    for (let x = x1; x < x2; x++) {
      let base = HIDE_BROWN;
      const mott = valueNoise(x, y, 2);
      if (mott > 0.6) base = HIDE_DARK;
...
for (const el of builtElements) {
  for (const face of faceOrder) {
    ...
    if (el.name === "crest") paintFlat(rect, CREST, 6);
    else if (el.name === "tuskL" || el.name === "tuskR") paintFlat(rect, TUSK, 4);
    else paintHide(rect);
  }
}

for (const el of builtElements) {
  if (el.name === "tuskL" || el.name === "tuskR" || el.name === "crest") continue;
...
for (let i = 0; i < canvas.length; i += 4) {
  if (canvas[i + 3] !== 255) {
    canvas[i] = HIDE_BROWN[0];
    canvas[i + 1] = HIDE_BROWN[1];
    canvas[i + 2] = HIDE_BROWN[2];
    canvas[i + 3] = 255;
  }
}
