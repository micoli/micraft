...
class ProceduralChunkGenerator(
    ...
    override fun generate(pos: ChunkPos): Chunk {
        ...
        for (lx in 0 until s) {
            for (ly in 0 until Chunk.SIZE_Y) {
                for (lz in 0 until s) {
                    blocks[Chunk.index(lx, ly, lz)] = terrainBlock(ly, cols[lx][lz]).ordinal.toByte()
                }
            }
        }
    ...
    private fun vegetationHash(wx: Int, wz: Int, typeIdx: Int): Double {
        var h = seed xor (wx.toLong() * 2654435761L) xor (wz.toLong() * 2246822519L) xor (typeIdx.toLong() * 1234567891L)
    ...
    private fun placeStructure(
        ...
        when (type) {
            "oak_tree"      -> if (surfaceBlock != BlockType.SAND && surfaceBlock != BlockType.SANDSTONE)
                placeOakTree(blocks, ox, oz, wx, wz, surfaceY)
            "pine_tree"     -> if (surfaceBlock != BlockType.SAND && surfaceBlock != BlockType.SANDSTONE)
                placePineTree(blocks, ox, oz, wx, wz, surfaceY, BlockType.PINE_LEAVES)
            "pine_tree_snow" -> if (surfaceBlock != BlockType.SAND && surfaceBlock != BlockType.SANDSTONE)
                placePineTree(blocks, ox, oz, wx, wz, surfaceY, BlockType.PINE_LEAVES_SNOW)
            "flower" -> if (surfaceBlock == BlockType.GRASS)
                setVeg(blocks, ox, oz, wx, surfaceY + 1, wz, BlockType.FLOWER)
            "weed"   -> if (surfaceBlock == BlockType.GRASS)
    ...
    private fun setVeg(blocks: ByteArray, ox: Int, oz: Int, wx: Int, wy: Int, wz: Int, type: BlockType) {
        val lx = wx - ox
        val lz = wz - oz
