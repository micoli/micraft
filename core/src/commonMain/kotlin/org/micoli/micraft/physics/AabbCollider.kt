...
object AabbCollider {
    ...
    /**
     * Int range covering blocks between [min, max). The -0.001 avoids counting a block when the
     * AABB face sits exactly on its boundary (touching ≠ inside).
     */
    ...
    fun isGrounded(
        ...
        for (bx in blocks(cx - hw, cx + hw)) for (bz in blocks(cz - hw, cz + hw)) if (isSolid(
            bx, by, bz))
            return true
        return false
    }
    ...
    /**
     * Resolve X movement against the world. Sweeps all intermediate columns to prevent tunneling at
     * high speed.
     */
    fun resolveX(
        isSolid: (Int, Int, Int) -> Boolean,
    ...
    /**
     * Resolve Z movement against the world. Sweeps all intermediate columns to prevent tunneling at
     * high speed.
     */
    fun resolveZ(
        isSolid: (Int, Int, Int) -> Boolean,
    ...
    fun resolveY(
        ...
        if (dy < 0) {
            ...
            for (by in from downTo to) for (bx in xs) for (bz in zs) if (isSolid(bx, by, bz))
                return ((by + 1f) - cy).coerceAtMost(0f)
        } else {
            val from = floor((cy + h).toDouble()).toInt()
            val to = floor((cy + dy + h).toDouble()).toInt()
            for (by in from..to) for (bx in xs) for (bz in zs) if (isSolid(bx, by, bz))
    ...
    fun canAdoptStance(
        ...
        for (by in from..to) for (bx in blocks(cx - hw, cx + hw)) for (bz in
            blocks(cz - hw, cz + hw)) if (isSolid(bx, by, bz)) return false
        return true
    }
}
