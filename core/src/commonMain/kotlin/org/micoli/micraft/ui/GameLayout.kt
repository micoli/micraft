...
@Serializable
enum class WidgetType { MINIMAP, HUD, SHORTCUT_BAR, CHAT_HISTORY, INPUT_BOX }
...
@Serializable
data class GameLayout(val name: String, val widgets: List<LayoutWidget>)
...
@Serializable
data class LayoutSyncPayload(val layouts: List<GameLayout>, val activeLayout: String)
...
fun validateLayouts(layouts: List<GameLayout>, activeLayout: String): String? {
    ...
    if (layouts.none { it.name == activeLayout }) return "Active layout '$activeLayout' not found in layouts"
    return null
}
